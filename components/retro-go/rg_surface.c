...
bool rg_surface_save_image_file(const rg_surface_t *source, const char *filename, int width, int height)
{
    ...
    // Заменяем кодирование PNG на наше быстрое Striped LZ
...
bool rg_surface_save_stlz(const rg_surface_t *source, const char *filename) {
    ...
    int stripe_h = 8; // Оптимальная высота полосы кадра (всего 8 линий)
    ...
    // Записываем черновой заголовок
    if (fwrite(&hdr, 1, sizeof(hdr), out) != sizeof(hdr)) {
        fclose(out);
        return false;
    }

    // Резервируем таблицу смещений полос
...
//    stlz_hash_t *hash = malloc(4096 * sizeof(stlz_hash_t));
    ...
    for (int i = 0; i < num_stripes; i++) {
        ...
        // Препроцессинг Delta-XOR по всей полосе. Смещение = длина одной строки
        stlz_delta_encode(stripe_buf, stripe_bytes, source->width * 2);

        // Сброс хэш-таблицы для независимого сжатия блоков
      //  memset(hash, 0xFF, 4096 * sizeof(stlz_hash_t));
    ...
    // Возвращаемся назад и дописываем заполненную таблицу смещений
