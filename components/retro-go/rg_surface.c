...
rg_surface_t *rg_surface_load_image(const uint8_t *data, size_t data_len, uint32_t flags)
{
    ...
    // STLZ: наш нативный формат (единственный поддерживаемый)
    if (memcmp(data, "STLZ", 4) == 0)
    {
        stlz_header_t *hdr = (stlz_header_t *)data;
        rg_surface_t *surf = rg_surface_create(hdr->width, hdr->height, RG_PIXEL_565_LE, 0);
        if (!surf) return NULL;
        
        uint32_t *offsets = (uint32_t *)(data + sizeof(stlz_header_t));
        uint32_t stripe_bytes = hdr->width * hdr->stripe_h * 2;
        
        uint8_t *stripe_buf = malloc(stripe_bytes);
        if (!stripe_buf) { rg_surface_free(surf); return NULL; }
        
        for (int s = 0; s < hdr->num_stripes; s++) {
            uint32_t offset = offsets[s];
            uint32_t comp_size = (s == hdr->num_stripes - 1) ? 
                                 (uint32_t)(data_len - offset) : (offsets[s+1] - offset);
            
            stlz_mem_reader_t r = { .buf = data + offset, .pos = 0, .size = comp_size };
            ...
            int y_start = s * hdr->stripe_h;
            int actual_h = RG_MIN(hdr->stripe_h, hdr->height - y_start);
            uint8_t *dst = surf->data + surf->offset + (y_start * surf->stride);
            
            memcpy(dst, stripe_buf, actual_h * surf->stride);
        }
    ...
    // RAW565: запасной вариант
    if (data_len == (data16[0] * data16[1] * 2 + 4))
    {
...
{
    CHECK_SURFACE(source, false);
    return rg_surface_save_stlz(source, filename);
}

bool rg_surface_save_stlz(const rg_surface_t *source, const char *filename) {
    FILE *out = fopen(filename, "wb");
    if (!out) return false;

    int stripe_h = 8;
    int num_stripes = (source->height + stripe_h - 1) / stripe_h;

    stlz_header_t hdr;
    memcpy(hdr.magic, STLZ_MAGIC, 4);
    hdr.width = source->width;
    hdr.height = source->height;
    hdr.stripe_h = stripe_h;
    hdr.num_stripes = num_stripes;
    hdr.format = source->format;
    ...
    uint32_t *offsets = malloc(num_stripes * sizeof(uint32_t));
    if (!offsets) { fclose(out); return false; }
    memset(offsets, 0, num_stripes * sizeof(uint32_t));
    if (fwrite(offsets, sizeof(uint32_t), num_stripes, out) != num_stripes) {
        free(offsets);
        fclose(out);
        return false;
    }

    stlz_hash_t *hash = malloc(STLZ_HASH_SIZE * sizeof(stlz_hash_t));
    uint32_t stripe_bytes = source->width * stripe_h * 2;
    uint8_t *stripe_buf = malloc(stripe_bytes);
    uint8_t *comp_buf = malloc(stripe_bytes + 256);
    ...
    for (int i = 0; i < num_stripes; i++) {
        ...
        if (y_end > source->height) y_end = source->height;
        int actual_h = y_end - y_start;

        uint8_t *dst_ptr = stripe_buf;
        for (int y = y_start; y < y_end; y++) {
            const uint8_t *src_ptr = (const uint8_t *)source->data + source->offset + (y * source->stride);
            memcpy(dst_ptr, src_ptr, source->width * 2);
            dst_ptr += source->width * 2;
        }
        if (actual_h < stripe_h) {
            memset(dst_ptr, 0, source->width * 2 * (stripe_h - actual_h));
        }

        stlz_delta_encode(stripe_buf, stripe_bytes, source->width * 2);
        memset(hash, 0xFF, STLZ_HASH_SIZE * sizeof(stlz_hash_t));

        stlz_mem_writer_t w = { .buf = comp_buf, .pos = 0, .max_size = stripe_bytes + 256 };
        stlz_write_lz77(stripe_buf, stripe_bytes, &w, hash);

        offsets[i] = (uint32_t)ftell(out);
        if (fwrite(comp_buf, 1, w.pos, out) != w.pos) {
            free(hash); free(stripe_buf); free(comp_buf); free(offsets);
            fclose(out);
            return false;
        }
