...
extern volatile uint32_t usb_kbd_gamepad_state;
...
#warning "not implemented"
    ...
    // --- НАШ ПЕРЕХВАТ (В САМОМ КОНЦЕ!) ---
    state |= usb_kbd_gamepad_state;
    // -------------------------------------
...
static void input_task(void *arg)
{
    ...
    while (input_task_running)
    {
        ...
        {
            // ОПТИМИЗАЦИЯ ИНПУТ-ЛАГА:
            // Вычитаем состояние USB-клавиатуры из аппаратных кнопок.
            // Кнопки консоли пойдут через медленный антидребезг (debounce),
            // а клавиатура пойдет в обход него с нулевой задержкой!
            uint32_t hw_state = state & ~usb_kbd_gamepad_state;

            for (int i = 0; i < RG_KEY_COUNT; ++i)
            {
                uint32_t val = ((debounce[i] << 1) | ((hw_state >> i) & 1));
                debounce[i] = val & 0xFF;
            ...
            // Объединяем очищенные кнопки консоли и мгновенную клавиатуру.
            gamepad_state = local_gamepad_state | usb_kbd_gamepad_state;
        }
        ...
        rg_task_delay(10);  // 10 (100 Гц) Опрашивает кнопки каждые 10 мс (100 Гц)
    }
...
void rg_input_init(void)
{
    ...
    //
    usb_kbd_init(); // Запускаем наш USB-драйвер
}
