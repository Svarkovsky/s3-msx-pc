...
/**
 * Render STLZ-compressed image directly from file.
 * Decodes and scales on-the-fly using only stack buffers (~10KB).
 * No heap allocation for framebuffer — safe for low-memory contexts.
 */
bool rg_gui_draw_stlz_image(int x_pos, int y_pos, int dest_w, int dest_h, const char *filepath);

/**
 * Render STLZ-compressed image directly from memory buffer.
 * Identical to rg_gui_draw_stlz_image but reads from flash/ROM
 * pointer instead of file. Decodes and scales on-the-fly using
 * only stack buffers (~10KB). Zero heap allocation for framebuffer
 * — safe for built-in assets and low-memory contexts.
 */
