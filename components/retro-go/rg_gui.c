...
#include "ds-stlz.h"
bool rg_gui_draw_stlz_image(int x_pos, int y_pos, int dest_w, int dest_h, const char *filepath);
bool rg_gui_draw_stlz_image_mem(int x_pos, int y_pos, int dest_w, int dest_h, const uint8_t *file_data);
...
bool rg_gui_set_theme(const char *theme_name)
{
    ...
    // Очищаем текущую тему (как и было ранее)
    cJSON_Delete(gui.theme_obj);
    gui.theme_obj = NULL;

    // Если передано NULL, мы расцениваем это как запрос
    // на полную выгрузку ресурсов GUI из кучи (деинициализацию)
    if (theme_name == NULL)
    {
        if (gui.draw_buffer)
        {
...
bool rg_gui_set_font(int index)
{
    ...
    // (Берем реальную высоту шрифта без всяких хаков):
    gui.font_height = gui.font->height; 
...
void rg_gui_draw_hourglass(void)
{
    rg_display_write_rect(
        ...
        gui.screen_height - image_hourglass.height - 30, // Опускаем вниз
...
void rg_gui_draw_status_bars(void)
{
 ...
 /*   snprintf(header, max_len, "SPEED: %d%% (%d %d) / BUSY: %d%%",
    ...
    snprintf(header, max_len, "S3-MSX-PC");
...
static rg_gui_event_t wifi_enable_cb(rg_gui_option_t *option, rg_gui_event_t event)
{
    ...
    if (event == RG_DIALOG_PREV || event == RG_DIALOG_NEXT || event == RG_DIALOG_ENTER)
    {
        ...
        return RG_DIALOG_UPDATE;  // обновить диалог без перерисовки фона
    }
    strcpy(option->value, enabled ? _("On") : _("Off"));
    return RG_DIALOG_VOID;
}
...
void rg_gui_options_menu(void)
{
    ...
    {
        options[count++] = (rg_gui_option_t){0, _("Volume"), "-", RG_DIALOG_FLAG_NORMAL, &volume_update_cb};
    }
    ...
    const rg_app_t *app = rg_system_get_app();
    if (app->isLauncher)
        memcpy(options + count, misc_options, sizeof(misc_options));
    else
        memcpy(options + count, game_options, sizeof(game_options));
...
void rg_gui_about_menu(void)
{
    ...
    rg_gui_option_t options[20] = {
        ...
        // {2, _("Debug menu"), NULL, RG_DIALOG_FLAG_NORMAL, NULL},  // DISABLED: MSX-PC
    ...
    while (true)
    {
        switch (rg_gui_dialog(_("About"), options, 4))
        {
            case 1:
                ...
                break;
            case 2:
                // rg_gui_debug_menu();  // DISABLED: MSX-PC
                break;
            case 3:
...
/**
 * Draw image with nearest-neighbor scaling using single scanline buffer.
 * Allocates only dest_w × 2 bytes on heap (≈1.3KB for 640px) instead of
 * full resized framebuffer. Safe fallback for legacy PNG previews when
 * PSRAM is critically low.
 */
static void rg_gui_draw_image_fit(int x_pos, int y_pos, int dest_w, int dest_h, const rg_surface_t *img) {
    if (!img || !img->data) return;
    
    x_pos = get_horizontal_position(x_pos, dest_w);
    y_pos = get_vertical_position(y_pos, dest_h);
    
    uint16_t *line_buffer = malloc(dest_w * sizeof(uint16_t));
    if (!line_buffer) return;
    
    bool swap_bytes = (img->format == RG_PIXEL_565_BE);
    
    for (int y = 0; y < dest_h; y++) {
        int y_src = (y * img->height) / dest_h;
        if (y_src >= img->height) y_src = img->height - 1;
        
        // Рассчитываем точный адрес строки источника с учетом смещения
        const uint16_t *src_line = (const uint16_t *)((const uint8_t *)img->data + img->offset + (y_src * img->stride));
        
        for (int x = 0; x < dest_w; x++) {
            int x_src = (x * img->width) / dest_w;
            if (x_src >= img->width) x_src = img->width - 1;
        ...
        rg_gui_copy_buffer(x_pos, y_pos + y, dest_w, 1, 0, line_buffer, false);
    }
...
/**
 * Save state slot selection callback.
 * Renders slot preview using STLZ (zero heap) when available,
 * falls back to scanline-based PNG rendering to avoid large
 * framebuffer allocations that would cause OOM with big disk images.
 */
static rg_gui_event_t slot_select_cb(rg_gui_option_t *option, rg_gui_event_t event)
{
    ...
    if (event == RG_DIALOG_FOCUS_GAINED)
    {
        ...
        if (slot->is_used)
        {
            // Проверяем, записан ли скриншот в новом легком формате STLZ
            if (slot->preview[0] && is_stlz_file(slot->preview))
            {   // Быстрый декодер STLZ (0 байт кучи под картинку)
                rg_gui_draw_stlz_image(0, margin, gui.screen_width, gui.screen_height - margin * 2, slot->preview);
            }
            else
            {
                ...
                if (preview)
                {
                    rg_gui_draw_image_fit(0, margin, gui.screen_width, gui.screen_height - margin * 2, preview);
                    rg_surface_free(preview);
                    preview = NULL; // Защита от double-free
                }
                else
                {
                    // Fallback на черный экран, если файл поврежден
                    rg_gui_draw_rect(0, margin, gui.screen_width, gui.screen_height - margin * 2, 0, C_BLACK, C_BLACK);
                }
            }
...
void rg_gui_game_menu(void)
{
    ...
    const rg_gui_option_t choices[] = {
        ...
        {3001, _("Load State"),      NULL, RG_DIALOG_FLAG_NORMAL, NULL},
        {3000, _("Reset"),           NULL, RG_DIALOG_FLAG_NORMAL, NULL},
        {4000, _("Take Screenshot"), NULL, RG_DIALOG_FLAG_NORMAL, NULL},  // 
    ...
    sel = rg_gui_dialog("S3-MSX-PC", choices, 0);
    ...
    switch (sel)
    {
        ...
        case 4000:
        {
            ...
            const char *rom_name = rg_basename(rg_system_get_app()->romPath);
            char rom_base[128];
            strncpy(rom_base, rom_name, sizeof(rom_base) - 1);
            char *dot = strrchr(rom_base, '.');
            if (dot) *dot = '\0';
            
            snprintf(path, sizeof(path), RG_BASE_PATH_SCREENSHOTS "/%s_%d.png", rom_base, (int)time(NULL));
            rg_emu_screenshot(path, 0, 0);
            rg_gui_draw_message(_("Screenshot Saved"));
            rg_task_delay(1000);
            break;
        }
...
/**
 * Decode and render STLZ image with nearest-neighbor scaling.
 * Reads compressed file into heap (~15-40KB), decodes one stripe
 * at a time into heap buffer (~4KB), scales one scanline into
 * heap buffer (~1.3KB). No full framebuffer allocation — peak
 * heap usage ~35KB vs ~630KB for legacy PNG path.
 */
bool rg_gui_draw_stlz_image(int x_pos, int y_pos, int dest_w, int dest_h, const char *filepath) {
    ...
    int stripe_h = hdr->stripe_h;
    int num_stripes = hdr->num_stripes;
    uint32_t stripe_bytes = hdr->width * stripe_h * 2;

    uint8_t *stripe_buf = malloc(stripe_bytes);
    if (!stripe_buf) { 
        free(file_data); 
        return false; 
    }
    ...
    int current_loaded_stripe_idx = -1;
    bool swap_bytes = (hdr->format == RG_PIXEL_565_BE);

    // Временный буфер масштабирования одной линии вывода (выделяем на куче вместо стека)
    uint16_t *line_buffer = malloc(dest_w * sizeof(uint16_t));
    if (!line_buffer) { 
        free(stripe_buf); 
        free(file_data); 
    ...
    for (int y_dest = 0; y_dest < dest_h; y_dest++) {
        ...
        int stripe_idx = y_src / stripe_h;
        int line_in_stripe = y_src % stripe_h;

        if (stripe_idx != current_loaded_stripe_idx) {
            uint32_t offset = offsets[stripe_idx];
            uint32_t comp_size = (stripe_idx == num_stripes - 1) ? 
                                 (file_size - offset) : (offsets[stripe_idx + 1] - offset);

            stlz_mem_reader_t r = { .buf = file_data + offset, .pos = 0, .size = comp_size };
            if (!stlz_read_lz77(stripe_buf, stripe_bytes, &r)) {
                free(line_buffer); free(stripe_buf); free(file_data); return false;
            }

            // Обратная дельта-XOR декодирование на месте
            stlz_delta_decode(stripe_buf, stripe_bytes, hdr->width * 2);
            current_loaded_stripe_idx = stripe_idx;
        }

        uint16_t *src_line = (uint16_t *)(stripe_buf + line_in_stripe * hdr->width * 2);

        // Nearest Neighbor масштабирование + Byte-Swap (BE -> LE) на лету за 1 проход
        for (int x = 0; x < dest_w; x++) {
        ...
        // Прямой вывод строки без выделения кадра на куче
        rg_gui_copy_buffer(x_pos, y_pos + y_dest, dest_w, 1, 0, line_buffer, false);
    }
...
/**
 * Render STLZ-compressed image directly from flash/ROM memory.
 * Identical pipeline to file version but reads from pointer.
 * Decodes one stripe (~4KB) and scales one scanline (~1.3KB) 
 * on-the-fly. Zero framebuffer allocation — peak heap ~6KB.
 * Used for built-in assets to eliminate PSRAM usage.
 */
bool rg_gui_draw_stlz_image_mem(int x_pos, int y_pos, int dest_w, int dest_h, const uint8_t *file_data) {
    ...
    uint16_t *line_buffer = malloc(dest_w * sizeof(uint16_t));
    if (!line_buffer) { free(stripe_buf); return false; }
    ...
    for (int y_dest = 0; y_dest < dest_h; y_dest++) {
        ...
        if (stripe_idx != current_loaded_stripe_idx) {
            uint32_t offset = offsets[stripe_idx];
            // Поскольку мы читаем из памяти, передаем огромный "размер" - чтение остановится по LZ77 потоку
            stlz_mem_reader_t r = { .buf = file_data + offset, .pos = 0, .size = 0xFFFFFFFF };
            if (!stlz_read_lz77(stripe_buf, stripe_bytes, &r)) {
                free(line_buffer); free(stripe_buf); return false;
            }
