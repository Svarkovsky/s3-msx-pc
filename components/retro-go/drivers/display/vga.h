...
//#define VGA_PCLK    25175000

#define VGA_PCLK    24000000     // 240 / 10  //#define VGA_PCLK    26666666     // 160 МГц / 6 
...
// =========================================================================
// ВЫНОСИМ БУФЕР В DRAM:
// Принудительно размещаем буфер во внутренней памяти процессора.
// Теперь Retro-Go пишет в него на максимальной скорости, не трогая PSRAM.
// Выравнивание 64 байта нужно для оптимальной работы кэш-линий.
// =========================================================================
//static uint16_t DRAM_ATTR __attribute__((aligned(64))) sram_render_buffer[MAX_CHUNK_PIXELS];
// Открываем буфер миру 
...
static void lcd_init(void) {
    ...
    esp_lcd_rgb_panel_config_t panel_config = {
        ...
        .num_fbs = 1,                                
        .bounce_buffer_size_px = 10 * VGA_WIDTH,     // 10
  //      .clk_src = LCD_CLK_SRC_DEFAULT,
        .clk_src = LCD_CLK_SRC_PLL240M, //   240 МГц
        .disp_gpio_num = -1,
        .pclk_gpio_num = 45,
    ...
    // =====================================================================
    // АППАРАТНЫЙ БАЙТ-СВАП:
    // Заставляем контроллер LCD_CAM аппаратно переворачивать байты во время чтения из видеопамяти. 
    // =====================================================================
...
// Отдаем фреймворку Retro-Go наш быстрый буфер из внутренней DRAM
...
static inline void lcd_send_buffer(uint16_t *buffer, size_t length) {
    ...
    for (int i = 0; i < lines; i++) {
        ...
        // ЧИСТЫЙ КОПИПАСТ ВМЕСТО МАТЕМАТИКИ:
        // Так как LCD_CAM сам перевернет байты, мы делаем обычный сверхбыстрый memcpy
        memcpy(dst, &buffer[i * window_w], line_bytes);
        
        // (QoS):
        // Остановка CPU на 3 микросекунды дает контроллеру VGA DMA 
        // окно для скачивания данных из PSRAM. Защитита от появления черных полос в тяжелых сценах.
        //esp_rom_delay_us(3);
    }
