...
rg_app_t *rg_system_init(int sampleRate, const rg_handlers_t *handlers, void *_unused)
{
    ...
    // ФИЗИЧЕСКИ ОТКЛЮЧАЕМ ДИСПЛЕЙ НА ВСЮ ИНИЦИАЛИЗАЦИЮ
    rg_display_off();
    ...
    // ФИЗИЧЕСКИ ВКЛЮЧАЕМ ДИСПЛЕЙ ПОСЛЕ ИНИЦИАЛИЗАЦИИ
    rg_display_on();
...
{
    // ФИЗИЧЕСКИ ОТКЛЮЧАЕМ ДИСПЛЕЙ НА ВСЁ ВРЕМЯ ВЫКЛЮЧЕНИЯ
    rg_display_off();
    ...
    // Освобождаем cJSON-структуру темы и буфер рисования GUI
    // через уже имеющийся и задекларированный интерфейс
...
bool rg_emu_save_state(uint8_t slot)
{
    ...
    // СНАЧАЛА скриншот — пока XBuf ещё не испорчен хэш-таблицей DS-LZ
    char *screenshot_path = rg_emu_get_path(RG_PATH_SCREENSHOT + slot, app.romPath);
    rg_emu_screenshot(screenshot_path, rg_display_get_width() / 2, 0);
    free(screenshot_path);

    // ПОТОМ сохранение стейта — XBuf может быть испорчен, но скриншот уже готов
