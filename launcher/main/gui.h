...
/**
 * Render STLZ preview image from file with size constraints.
 * Decodes and scales on-the-fly using only stack buffers (~10KB).
 * Image is automatically fitted within max_width × max_height bounds.
 * Zero heap allocation for framebuffer — safe for slot previews.
 */
bool gui_draw_stlz_preview(const char *filepath, int max_width, int max_height);

/**
 * Render STLZ-compressed image directly from flash/ROM memory.
 * Reads from pointer instead of file — used for built-in assets.
 * Decodes and scales on-the-fly using only stack buffers (~10KB).
 * Zero heap allocation for framebuffer — safe for low-memory contexts.
 */
