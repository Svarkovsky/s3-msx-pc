...
#define SETTING_SCREENSAVER   "Screensaver"
#define SETTING_SCREENSAVER_TO "ScreensaverTO"

static rg_gui_event_t screensaver_enable_cb(rg_gui_option_t *option, rg_gui_event_t event)
{
    int val = rg_settings_get_number(NS_APP, SETTING_SCREENSAVER, 1);
    if (event == RG_DIALOG_PREV || event == RG_DIALOG_NEXT || event == RG_DIALOG_ENTER)
    {
        val = !val;
        rg_settings_set_number(NS_APP, SETTING_SCREENSAVER, val);
        rg_settings_commit();
    }
    strcpy(option->value, val ? _("On") : _("Off"));
    return RG_DIALOG_VOID;
}

static rg_gui_event_t screensaver_timeout_cb(rg_gui_option_t *option, rg_gui_event_t event)
{
    int val = rg_settings_get_number(NS_APP, SETTING_SCREENSAVER_TO, 600);   // 600
    int step = (event == RG_DIALOG_PREV) ? -60 : 60;
    ...
    if (event == RG_DIALOG_PREV || event == RG_DIALOG_NEXT)
    {
        ...
        if (val < 0) val = 0;
        if (val > 999) val = 999;
        rg_settings_set_number(NS_APP, SETTING_SCREENSAVER_TO, val);
        rg_settings_commit();
    }
    ...
    if (val == 0)
        ...
        sprintf(option->value, "%d sec", val);
    return RG_DIALOG_VOID;
}
...
{
    const char *modes[] = {_("Last app"), _("Launcher")};
    int max = 1;
...
{
    bool enabled = rg_settings_get_number(NS_APP, SETTING_WEBUI, 1);
    if (event == RG_DIALOG_PREV || event == RG_DIALOG_NEXT || event == RG_DIALOG_ENTER)
    {
        enabled = !enabled;
        webui_stop();
...
static void retro_loop(void)
{
    ...
    while (true)
    {
        ...
        else if (rg_system_timer() >= next_idle_event)
        {
            ...
            int ss_enable = rg_settings_get_number(NS_APP, SETTING_SCREENSAVER, 1);
            int ss_timeout = rg_settings_get_number(NS_APP, SETTING_SCREENSAVER_TO, 30);

            if (ss_enable && ss_timeout > 0 && gui.idle_counter > ss_timeout * 10)
            {
                run_screensaver();
                gui.idle_counter = 0;
                redraw_pending = true;
...
static rg_gui_event_t hw_model_cb(rg_gui_option_t *option, rg_gui_event_t event)
{
    int model = rg_settings_get_number("msx", "HWModel", 1);
    int prev_model = model;

    if (event == RG_DIALOG_PREV && --model < 0) model = 2;
    if (event == RG_DIALOG_NEXT && ++model > 2) model = 0;

    if (model != prev_model) {
        rg_settings_set_number("msx", "HWModel", model);
        rg_settings_commit();
    }

    if (model == 0)      strcpy(option->value, "MSX1");
    else if (model == 1) strcpy(option->value, "MSX2");
    else                 strcpy(option->value, "MSX2+");

    return RG_DIALOG_VOID;
}

static rg_gui_event_t disk_autosave_cb(rg_gui_option_t *option, rg_gui_event_t event)
{
    int autosave = rg_settings_get_number("msx", "DiskAutoSave", 0);

    if (event == RG_DIALOG_PREV || event == RG_DIALOG_NEXT || event == RG_DIALOG_ENTER) {
        autosave = !autosave;
        rg_settings_set_number("msx", "DiskAutoSave", autosave);
        rg_settings_commit();
    }

    strcpy(option->value, autosave ? _("On") : _("Off"));
    return RG_DIALOG_VOID;
}

static rg_gui_event_t auto_fire_cb(rg_gui_option_t *option, rg_gui_event_t event)
{
    int autofire = rg_settings_get_number("msx", "AutoFire", 0);

    if (event == RG_DIALOG_PREV || event == RG_DIALOG_NEXT || event == RG_DIALOG_ENTER) {
        autofire = !autofire;
        rg_settings_set_number("msx", "AutoFire", autofire);
        rg_settings_commit();
    }

    strcpy(option->value, autofire ? _("On") : _("Off"));
    return RG_DIALOG_VOID;
}
...
static void options_handler(rg_gui_option_t *dest)
{
    const rg_gui_option_t options[] = {
        {0, _("Hardware Model"),  "-", RG_DIALOG_FLAG_NORMAL, &hw_model_cb},
        {0, _("Disk Auto-Save"),  "-", RG_DIALOG_FLAG_NORMAL, &disk_autosave_cb},
        {0, _("Auto Fire"),       "-", RG_DIALOG_FLAG_NORMAL, &auto_fire_cb},
        {0, _("Screensaver"),     "-", RG_DIALOG_FLAG_NORMAL, &screensaver_enable_cb},
        {0, _("Scr. Timeout"),    "-", RG_DIALOG_FLAG_NORMAL, &screensaver_timeout_cb},
