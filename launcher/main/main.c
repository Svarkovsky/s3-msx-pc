...
{
    int model = rg_settings_get_number("msx", "HWModel", 1); // 1 = MSX2 default
...
{
    // Читаем текущее значение из пространства msx (по умолчанию 0 - Off)
    int autosave = rg_settings_get_number("msx", "DiskAutoSave", 0);

    if (event == RG_DIALOG_PREV || event == RG_DIALOG_NEXT || event == RG_DIALOG_ENTER) {
        autosave = !autosave;
        rg_settings_set_number("msx", "DiskAutoSave", autosave);
        rg_settings_commit(); // Сохраняем изменения в msx.json
    }
...
void app_main(void)
{
    ...
    // Усыпляем основной поток на полсекунды. 
    // За это время фоновая задача USB получит 100% процессора 
    // и гарантированно завершит энумерацию клавиатуры.
    rg_task_delay(500);
    // ==========================================
