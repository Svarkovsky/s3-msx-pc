...
#include <cJSON.h>
#include <ctype.h>
#include <ff.h> // для работы с FATFS
...
static esp_err_t http_api_handler(httpd_req_t *req)
{
    ...
    if (strcmp(cmd, "list") == 0)
    {
        ...
        // Получаем информацию об объеме SD-карты через FATFS ---
        FATFS *fs;
        DWORD fre_clust;

        // Пытаемся получить информацию о файловой системе для пути
        FRESULT res = f_getfree(arg1, &fre_clust, &fs);
        if (res == FR_OK && fs != NULL) {
            // Общее количество кластеров = (fs->n_fatent - 2) исключая зарезервированные
            DWORD tot_clust = (fs->n_fatent - 2);
            // Размер сектора в байтах. В ESP-IDF с wear levelling это обычно 4096,
            // но для надежности запрашиваем через fs->ssize или используем константу 512/4096
            WORD sect_size = fs->ssize;  // ИСПОЛЬЗУЕМ ПОЛЕ СТРУКТУРЫ НАПРЯМУЮ
            DWORD sect_per_clust = fs->csize;
            
            double total_bytes = (double)tot_clust * sect_per_clust * sect_size;
            double free_bytes = (double)fre_clust * sect_per_clust * sect_size;
            
            cJSON_AddNumberToObject(response, "total_bytes", total_bytes);
            cJSON_AddNumberToObject(response, "free_bytes", free_bytes);
        }
    }
...
static esp_err_t http_download_handler(httpd_req_t *req)
{
    ...
    if ((fp = fopen(filename, "rb")))
    {
        ...
        char header_val[512];
        snprintf(header_val, sizeof(header_val), "attachment; filename=\"%s\"", base_name);
