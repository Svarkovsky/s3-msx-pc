...
#include "ds-stlz.h" 
static char current_preview_path[RG_PATH_MAX] = {0}; // Буфер текущего STLZ превью
...
void gui_init(bool cold_boot)
{
    gui = (retro_gui_t){
     ...
     //   .show_preview = rg_settings_get_number(NS_APP, SETTING_SHOW_PREVIEW, PREVIEW_MODE_SAVE_COVER),
        .show_preview = rg_settings_get_number(NS_APP, SETTING_SHOW_PREVIEW, PREVIEW_MODE_SAVE_ONLY),
...
// В gui_update_theme() заменить RG_COUNT(gui.themes) на THEME_COUNT и добавить тему 5:

void gui_update_theme(void)
{
    // Тема 1: Default (белый выделенный)
    ...
    gui.themes[3].list.selected_bg = rg_gui_get_theme_color("launcher_4", "list_selected_bg", C_CYAN);
    gui.themes[3].list.selected_fg = rg_gui_get_theme_color("launcher_4", "list_selected_fg", C_BLACK);

    // Тема 5: Red (как тема 2, но красный)
    gui.themes[4].background = rg_gui_get_theme_color("launcher_5", "background", C_BLACK);
    gui.themes[4].foreground = rg_gui_get_theme_color("launcher_5", "foreground", C_SNOW);
    gui.themes[4].list.standard_bg = rg_gui_get_theme_color("launcher_5", "list_standard_bg", C_TRANSPARENT);
    gui.themes[4].list.standard_fg = rg_gui_get_theme_color("launcher_5", "list_standard_fg", C_GRAY);
    gui.themes[4].list.selected_bg = rg_gui_get_theme_color("launcher_5", "list_selected_bg", C_TRANSPARENT);
    gui.themes[4].list.selected_fg = rg_gui_get_theme_color("launcher_5", "list_selected_fg", C_RED);
...
/**
 * Draw tab preview image in bottom-right corner.
 * Supports both STLZ (on-the-fly decode+scale) and legacy PNG.
 * STLZ uses zero heap for decoded pixels; PNG falls back to
 * standard resampled rendering. Image is clipped to PREVIEW_WIDTH
 * × PREVIEW_HEIGHT maximum.
 */
void gui_draw_preview(tab_t *tab)
{
    if (current_preview_path[0])
    {
        int width = 0, height = 0;
        if (rg_stlz_get_dimensions(current_preview_path, &width, &height))
        {
         int draw_height = RG_MIN(height, PREVIEW_HEIGHT);  // макс. 70% высоты экрана
         int draw_width  = RG_MIN(width, PREVIEW_WIDTH);    // макс. 50% ширины экрана
            // Отрисовка превью в правый нижний угол без выделения памяти.
            rg_gui_draw_stlz_image(-draw_width, -draw_height, draw_width, draw_height, current_preview_path);
        }
    }
    else if (tab->preview)
    {
...
/**
 * Draw tab background with optional shading.
 * Priority: custom theme PNG (PSRAM) > built-in STLZ (flash, zero heap) > solid color.
 * Shading is applied only to loaded PNG backgrounds.
 * STLZ path renders on-the-fly without framebuffer allocation.
 */
void gui_draw_background(tab_t *tab, int shade)
{
    ...
    if (!tab->background && !gui.low_memory_mode)
    {
        tab->background = gui_get_image("background", tab->name);
        if (!tab->background)
            tab->background = gui_get_image("background", NULL);
    ...
    if (tab->background)
    {
        // Кастомный фон из темы (уже загружен в PSRAM)
        rg_gui_draw_image(0, 0, gui.width, gui.height, false, tab->background);
    }
    else if (!gui.browse)
    {
        // Дефолтный фон ТОЛЬКО для заглавного экрана (карусель)
        bool drawn = false;
        for (const binfile_t **img = builtin_images; *img; img++)
        {
            if (strncmp((*img)->name, "background", 10) == 0)
            {
                drawn = rg_gui_draw_stlz_image_mem(0, 0, gui.width, gui.height, (*img)->data);
                if (drawn) break;
            }
        }
        if (!drawn)
    ...
    {
        // Для списка файлов — черный фон (как было раньше)
        rg_gui_draw_rect(0, 0, gui.width, gui.height, 0, 0, gui.theme->background);
    }
}
...
/**
 * Set or clear tab preview image.
 * Frees previous preview if any. When setting a PNG preview,
 * clears the STLZ path buffer to avoid conflicts. Pass NULL
 * to release resources without replacing.
 */
void gui_set_preview(tab_t *tab, rg_image_t *preview)
{
    ...
    tab->preview = preview;
    current_preview_path[0] = '\0'; // Очищаем путь, если задействовано классическое превью
}

/**
 * Load preview image for selected list item.
 * Tries STLZ first (stores path only, zero pixel heap),
 * falls back to PNG loading. Skips preview entirely in
 * low-memory mode to preserve PSRAM for emulation.
 */
void gui_load_preview(tab_t *tab)
{
    ...
    uint32_t errors = 0;
    bool stlz_found = false;

    while (order && !tab->preview && !stlz_found)
    {
        ...
        if (path_len > 0 && path_len < RG_PATH_MAX)
        {
            ...
            // Check if this is an STLZ file first - zero heap memory for decoded image
            if (is_stlz_file(path))
            {
                strncpy(current_preview_path, path, sizeof(current_preview_path) - 1);
                current_preview_path[sizeof(current_preview_path) - 1] = '\0';
                stlz_found = true;
                file->missing_cover &= ~(1 << type); // Clear missing flag since we found it
                break;
            }
            else
            {
                // Fallback to regular PNG loading (uses heap memory)
                gui_set_preview(tab, rg_surface_load_image_file(path, 0));
            }
        }

        // Mark as missing only if we didn't find anything (STLZ or PNG)
        if (!tab->preview && !stlz_found)
        {
            file->missing_cover |= (1 << type);
            if (!tab->preview && rg_storage_exists(path))
                errors++;
        }
    }

    if (!tab->preview && !stlz_found && file->checksum && (show_missing_cover || errors))
    {
