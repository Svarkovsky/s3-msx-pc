/**
    ...
    Heavily optimized for ESP32-S3 (Xtensa) by Ivan Svarkovsky, 2026.
    ...
    - Secondary RAM Cartridge injected into Slot 3-3
     to bypass 4MB 8-bit Z80 mapper limit. MSX-DOS automatically
     pools this with primary RAM for large RAM-disks.

    - Yamaha SFG-05 (OPM / YM2151) Dynamic Injection:
     Fakes an SFG cartridge presence with "MCHFM0" signature and
     intercepts MMIO at 0x3FF0-0x3FF1 to drive Sega OPN translations
     via VGMPlay MSX seamlessly. Powers down when inactive.
...
/* Безопасное определение типов для ESP-IDF / GCC */
#ifndef byte
...
// === YAMAHA SFG-05 (OPM) VIRUTAL MODULE ===================
byte *SFG_ROM = NULL;
int sfg_p = -1;
int sfg_s = -1;
byte sfg_addr = 0;
int sfg_active = 0; // 0 = спящий режим (отключен), 1 = активен
int opm_inited = 0; // 0 = DRAM свободна, 1 = чип инициализирован

extern int YM2151Init(int num, int clock, int rate);
extern void YM2151WriteReg(int n, int r, int v);
extern int YM2151ReadStatus(int n);
extern void YM2151ResetChip(int num);

// Анти-LTO обёртки: запрещаем затягивать код YM2151 внутрь IRAM-функций (мало места)
__attribute__((noinline)) static byte wrap_YM2151ReadStatus(void) {
    return YM2151ReadStatus(0);
}

__attribute__((noinline)) static void wrap_YM2151WriteReg(byte r, byte v) {
    YM2151WriteReg(0, r, v);
}
// ==========================================================
...
byte *RAMData2 = 0;       /* Secondary RAM (Slot 3-3) for RAM-Disk */
byte RAMMapper2[4] = {0,0,0,0}; /* Secondary RAM Mapper state */
byte RAMMask2 = 0;        /* Secondary RAM Mask */
int  RAMPages2 = 0;       /* Secondary RAM size in pages */
int  StateVersion = 3;    /* State file format version */
...
/*    if (FNTName) {
    ...
    if (FNTName) {
        J = LoadFNT(FNTName);
        if (J) {
            if (Verbose) printf("Loading %s font...OK\n", FNTName);
        }  // Если не загрузился — тихо используем встроенный шрифт
    }
    if (Verbose) {
...
void TrashMSX(void) {
    ...
    // === ОЧИСТКА РЕСУРСОВ YM2151 (SFG-05) ===
    extern void YM2151Shutdown(void);
    if (opm_inited) {
        YM2151Shutdown();
        opm_inited = 0;
        printf("YM2151: Heap resources freed successfully (DRAM returned)\n");
    }
    sfg_p = -1;
    sfg_s = -1;
    sfg_active = 0;
    SFG_ROM = NULL; // Обязательно NULL
    // ========================================
    ...
    FreeAllMemory(); // Освобождает блоки, выделенные через GetMemory (включая старый SFG_ROM)
}
/** ResetMSX() ***********************************************/
int ResetMSX(int NewMode, int NewRAMPages, int NewVRAMPages) {
    ...
    /* --- SECONDARY RAM FOR RAM-DISK (128 kB / 8 Pages) --- */
     if (RAMPages2 != 8) {
        if (Verbose) {
            printf("Allocating 128kB for Secondary RAM (RAM-Disk)...");
        }
        if ((P1 = GetMemory(8 * 0x4000))) {
            memset(P1, NORAM, 8 * 0x4000);
            FreeMemory(RAMData2);
            RAMPages2 = 8;
            RAMMask2 = 7;
            RAMData2 = P1;
    ...
    for (J = 0; J < 4; ++J) {
        ...
        if (RAMData2) {
            MemMap[3][3][J * 2] = RAMData2 + (3 - J) * 0x4000;
            MemMap[3][3][J * 2 + 1] = MemMap[3][3][J * 2] + 0x2000;
            RAMMapper2[J] = 3 - J;
        } else {
            MemMap[3][3][J * 2] = EmptyRAM;
            MemMap[3][3][J * 2 + 1] = EmptyRAM;
            RAMMapper2[J] = 0;
        }
    ...
    // --- Инициализация виртуального Yamaha SFG-05 (OPM) ---
    if (sfg_p == -1) {
        if (!SFG_ROM) SFG_ROM = GetMemory(0x4000);
        if (SFG_ROM) {
            memset(SFG_ROM, 0xFF, 0x4000);
            memcpy(&SFG_ROM[0x0080], "MCHFM0", 6); // Сигнатура для VGMPlay
            // Жестко привязываем SFG-05 к слоту 0-2 (Primary 0, Subslot 2).
            // Этот слот аппаратно расширяемый, а субслот 2 (Cartridge 4) полностью свободен. 
            // Это исключает любые конфликты с BIOS (0-0) и играми в слотах 1-0 и 2-0.
            sfg_p = 0; 
            sfg_s = 2;
            MemMap[sfg_p][sfg_s][0] = SFG_ROM;
            MemMap[sfg_p][sfg_s][1] = SFG_ROM + 0x2000;
        }
    }
    sfg_active = 0;
    sfg_addr = 0;
    // Сброс чипа будет вызван через ResetStudioChips -> YM2151ResetChip(0)
    // ------------------------------------------------------
    for (J = 0; J < MAXSLOTS; ++J)
        if ((I = ROMMask[J] + 1) > 4) {
...
IRAM_ATTR byte RdZ80(word A) {
    // === Хук для виртуального модуля SFG-05 ===
    if (UNLIKELY((A & 0x3FF8) == 0x3FF0)) {
        int page = A >> 14;
        if (PSL[page] == sfg_p && SSL[page] == sfg_s) {
            if ((A & 7) == 1) {
                // Инициализация при первом чтении статуса
                if (!opm_inited) {
                    YM2151Init(1, 4000000, 16000);
                    opm_inited = 1;
                    extern int pending_audio_samples;
                    pending_audio_samples = 0;
                    printf("YM2151: Lazy init OK (16kHz).\n");
                }
                return wrap_YM2151ReadStatus();
            }
            return 0xFF;
        }
    }
        ...
        if (PSL[page] == sfg_p && SSL[page] == sfg_s) {
            if ((A & 7) == 0) {
                ...
                if (!opm_inited) {
                    YM2151Init(1, 4000000, 16000);
                    opm_inited = 1;
                    // Сброс аудио-буфера, чтобы не копились старые сэмплы
                    extern int pending_audio_samples;
                    pending_audio_samples = 0;
                    printf("YM2151: Lazy init OK (16kHz).\n");
                }
                wrap_YM2151WriteReg(sfg_addr, V);
                sfg_active = 1;
            }
            return;
        }
...
void OutZ80(word Port, byte Value) {
    ...
    switch (Port) {
        ...
        case 0xFF:
            ...
            byte V1 = Value & RAMMask;
            if (RAMMapper[J] != V1) {
                if (Verbose & 0x08) {
                    printf("RAM-MAPPER: block %d at %Xh\n", V1, J * 0x4000);
                }
                RAMMapper[J] = V1;
                MemMap[3][2][I] = RAMData + ((int)V1 << 14);
            ...
            if (RAMData2) {
                byte V2 = Value & RAMMask2; // Value & 63.
                if (RAMMapper2[J] != V2) {
                    RAMMapper2[J] = V2;
                    MemMap[3][3][I] = RAMData2 + ((int)V2 << 14);
                    MemMap[3][3][I + 1] = MemMap[3][3][I] + 0x2000;
                    if ((PSL[J] == 3) && (SSL[J] == 3)) {
                        EnWrite[J] = 1;
                        RAM[I] = MemMap[3][3][I];
                        RAM[I + 1] = MemMap[3][3][I + 1];
                    }
                }
            }
            return;
...
void MapROM(register word A, register byte V) {
    ...
    if (CurrentSndMode == SND_MODE_ACCURATE && studio_scc) {
        if (A == 0x9000 || A == 0x9FFE || A == 0x9FFF || A == 0xBFFE || A == 0xBFFF
            || A == 0xB000 || ((A & 0xDF00) == 0x9800)) {
...
void PSlot(register byte V) {
    ...
    if (PSLReg != V)
        for (PSLReg = V, J = 0; J < 4; ++J, V >>= 2) {
            ...
            int is_ram1 = (PSL[J] == 3) && (SSL[J] == 2) && (MemMap[3][2][I] != EmptyRAM);
            int is_ram2 = (PSL[J] == 3) && (SSL[J] == 3) && (MemMap[3][3][I] != EmptyRAM);
            EnWrite[J] = is_ram1 || is_ram2;
        }
}
...
byte ChangeDisk(byte N, const char *FileName) {
    ...
    if (!FileName) {
        EjectFDI(&FDD[N]);
        return (1);
    } // Извлекаем старую дискету ПЕРЕД загрузкой новой.

    EjectFDI(&FDD[N]);  // Это немедленно высвобождает 720 КБ в PSRAM и предотвращает OOM.
    ...
    // Если загрузка нового образа не удалась, форматируем пустой диск (fallback-логика)
    P = FormatFDI(&FDD[N], FMT_MSXDSK);
    if (P && !(*FileName ? DSKLoad(FileName, P, "MSX-DISK")
...
/** MSX_SaveDisks() ******************************************/
/** Сохраняет изменённые образы дискет A и B.               **/
/** Вызывается при выходе из эмулятора, аппаратном сбросе,  **/
/** смене диска или по требованию.                          **/
/*************************************************************/
