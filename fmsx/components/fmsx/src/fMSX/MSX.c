...
int ResetMSX(int NewMode, int NewRAMPages, int NewVRAMPages) {
    ...
    /* --- SECONDARY RAM FOR RAM-DISK (1.0 MB / 64 Pages) --- */
    /* if (RAMPages2 != 64) {
        if (Verbose) {
            printf("Allocating 1024kB for Secondary RAM (RAM-Disk)...");
        }
        if ((P1 = GetMemory(64 * 0x4000))) {
            memset(P1, NORAM, 64 * 0x4000);
            FreeMemory(RAMData2);
            RAMPages2 = 64;
            RAMMask2 = 63;
            RAMData2 = P1;
    ...
    */
    /* --- SECONDARY RAM FOR RAM-DISK (512 kB / 32 Pages) --- */
    /*if (RAMPages2 != 32) {
        if (Verbose) {
            printf("Allocating 512kB for Secondary RAM (RAM-Disk)...");
        }
        if ((P1 = GetMemory(32 * 0x4000))) {
            memset(P1, NORAM, 32 * 0x4000);
            FreeMemory(RAMData2);
            RAMPages2 = 32;
            RAMMask2 = 31;
            RAMData2 = P1;
    ...
    /* --- SECONDARY RAM FOR RAM-DISK (256 kB / 16 Pages) --- */
    /* if (RAMPages2 != 16) {
        if (Verbose) {
            printf("Allocating 256kB for Secondary RAM (RAM-Disk)...");
        }
        if ((P1 = GetMemory(16 * 0x4000))) {
            memset(P1, NORAM, 16 * 0x4000);
            FreeMemory(RAMData2);
            RAMPages2 = 16;
            RAMMask2 = 15;
            RAMData2 = P1;
...
byte ChangeDisk(byte N, const char *FileName) {
    ...
    if (!FileName) {
        EjectFDI(&FDD[N]);
        return (1);
    } // ИСПРАВЛЕНИЕ: Извлекаем старую дискету ПЕРЕД загрузкой новой.
