/**
 * fMSX: portable MSX emulator — V9938.c
 *
 ...
 *
 * - All VDP engine functions (SrchEngine, LineEngine, LmmvEngine,
 *   LmmmEngine, LmcmEngine, LmmcEngine, HmmvEngine, HmmmEngine,
 *   YmmmEngine, HmmcEngine), VDPRead, VDPWrite, and LoopVDP
 *   moved to IRAM via IRAM_ATTR to eliminate Flash Cache misses.
 * - Delta timing values cached in MMC.delta (computed once in
 *   VDPDraw, reused in all engines).
 * - All inline helpers (VDPVRMP, VDPpoint*, VDPpset*, VDPpsetlowlevel)
 *   marked __attribute__((always_inline)) to guarantee inlining
 *   into IRAM engine functions, preventing flash calls from IRAM.
 * - Timing arrays (srch_timing, line_timing, etc.) changed to
 *   static const to move them from DRAM to Flash (.rodata).
 * - Centralized Indirect Branch Elimination: Replaced the VdpEngine
 *   function pointer with an integer ID (ActiveVdpEngine) and a single
 *   IRAM-allocated dispatcher RunActiveEngine(). This saves up to 400-500
 *   bytes of precious IRAM and prevents Xtensa pipeline stalls.
 * - Folded the logical operations switch in VDPpsetlowlevel from 13 down
 *   to 5 cases by handling OP >= 8 with a branchless guard, significantly
 *   improving jump table performance and reducing I-cache pressure.
 * - Branch Prediction: Added __builtin_expect hints in LoopVDP, VDPWrite,
 *   and VDPRead, reducing branch penalties on Xtensa.
 * - VDP Register Consolidation: Implemented VDP_R16() inline helper to
 *   collapse redundant 16-bit register reads, reducing register pressure.
 *
 * WARNING: This version is under active testing. Please report
 * any graphical glitches or performance regressions to:
...
static byte VDPpoint(register byte SM, register int SX, register int SY);

static void VDPpsetlowlevel(register byte *P,
                            register byte CL,
                            register byte M,
                            register byte OP);

static void
VDPpset5(register int DX, register int DY, register byte CL, register byte OP);
static void
VDPpset6(register int DX, register int DY, register byte CL, register byte OP);
static void
VDPpset7(register int DX, register int DY, register byte CL, register byte OP);
static void
VDPpset8(register int DX, register int DY, register byte CL, register byte OP);
...
static const int srch_timing[8] = {818,
                                   1025,
                                   818,
                                   830, /* ntsc */
                                   696,
...
__attribute__((always_inline)) static inline uint16_t VDP_R16(byte idx) {
    return (uint16_t)VDP[idx] | ((uint16_t)VDP[idx + 1] << 8);
}

/** VDPVRMP() **********************************************/
/** Calculate addr of a pixel in vram                       **/
/*************************************************************/
__attribute__((always_inline)) INLINE byte *VDPVRMP(byte M, int X, int Y) {
    switch (M) {
        case 0:
...
/*************************************************************/
__attribute__((always_inline)) INLINE byte VDPpoint6(int SX, int SY) {
...
/*************************************************************/
__attribute__((always_inline)) INLINE byte VDPpoint7(int SX, int SY) {
...
/*************************************************************/
__attribute__((always_inline)) INLINE byte VDPpoint8(int SX, int SY) {
...
/*************************************************************/
__attribute__((always_inline)) INLINE byte VDPpoint(byte SM, int SX, int SY) {
    switch (SM) {
        case 0:
            return VDPpoint5(SX, SY);
        case 1:
            return VDPpoint6(SX, SY);
        case 2:
            return VDPpoint7(SX, SY);
        case 3:
            return VDPpoint8(SX, SY);
    }
...
VDPpsetlowlevel(byte *P, byte CL, byte M, byte OP) {
    if (OP >= 8) {
        if (!CL)
            return;
        OP -= 8;
...
/*************************************************************/
__attribute__((always_inline)) INLINE void
VDPpset5(int DX, int DY, byte CL, byte OP) {
...
/*************************************************************/
__attribute__((always_inline)) INLINE void
VDPpset6(int DX, int DY, byte CL, byte OP) {
    register byte SH = ((~DX) & 3) << 1;

    VDPpsetlowlevel(VDP_VRMP6(DX, DY), CL << SH, ~(3 << SH), OP);
}
...
/*************************************************************/
__attribute__((always_inline)) INLINE void
VDPpset7(int DX, int DY, byte CL, byte OP) {
    register byte SH = ((~DX) & 1) << 2;

    VDPpsetlowlevel(VDP_VRMP7(DX, DY), CL << SH, ~(15 << SH), OP);
}
...
/*************************************************************/
__attribute__((always_inline)) INLINE void
VDPpset8(int DX, int DY, byte CL, byte OP) {
    VDPpsetlowlevel(VDP_VRMP8(DX, DY), CL, 0, OP);
}
...
/*************************************************************/
__attribute__((always_inline)) INLINE void
VDPpset(byte SM, int DX, int DY, byte CL, byte OP) {
    switch (SM) {
        case 0:
            VDPpset5(DX, DY, CL, OP);
            break;
        case 1:
            VDPpset6(DX, DY, CL, OP);
            break;
        case 2:
            VDPpset7(DX, DY, CL, OP);
            break;
        case 3:
            VDPpset8(DX, DY, CL, OP);
            break;
    }
}
...
static int GetVdpTimingValue(register int *timing_values) {
    return (timing_values[((VDP[1] >> 6) & 1) | (VDP[8] & 2)
                          | ((VDP[9] << 1) & 4)]);
}

/** RunActiveEngine() ****************************************/
/** Execute the currently active graphical engine           **/
/*************************************************************/
...
#define post_srch(MX)                                                          \
    ...
    switch (ScrMode) {
        case 5:
            pre_srch VDPpoint5(SX, SY) post_srch(256) break;
        case 6:
            pre_srch VDPpoint6(SX, SY) post_srch(512) break;
        case 7:
            pre_srch VDPpoint7(SX, SY) post_srch(512) break;
        case 8:
            pre_srch VDPpoint8(SX, SY) post_srch(256) break;
    }
...
#define post_lineymaj(MX)                                                      \
    ...
    if ((VDP[45] & 0x01) == 0)
        ...
        switch (ScrMode) {
            case 5:
                pre_loop VDPpset5(DX, DY, CL, LO);
                post_linexmaj(256) break;
            case 6:
                pre_loop VDPpset6(DX, DY, CL, LO);
                post_linexmaj(512) break;
            case 7:
                pre_loop VDPpset7(DX, DY, CL, LO);
                post_linexmaj(512) break;
            case 8:
                pre_loop VDPpset8(DX, DY, CL, LO);
                post_linexmaj(256) break;
        }
    else
        ...
        switch (ScrMode) {
            case 5:
                pre_loop VDPpset5(DX, DY, CL, LO);
                post_lineymaj(256) break;
            case 6:
                pre_loop VDPpset6(DX, DY, CL, LO);
                post_lineymaj(512) break;
            case 7:
                pre_loop VDPpset7(DX, DY, CL, LO);
                post_lineymaj(512) break;
            case 8:
...
IRAM_ATTR void LmmvEngine(void) {
    ...
    switch (ScrMode) {
        case 5:
            pre_loop VDPpset5(ADX, DY, CL, LO);
            post__x_y(256) break;
        case 6:
            pre_loop VDPpset6(ADX, DY, CL, LO);
            post__x_y(512) break;
        case 7:
            pre_loop VDPpset7(ADX, DY, CL, LO);
            post__x_y(512) break;
        case 8:
            pre_loop VDPpset8(ADX, DY, CL, LO);
            post__x_y(256) break;
    }
...
IRAM_ATTR void LmmmEngine(void) {
    ...
    switch (ScrMode) {
        case 5:
            pre_loop VDPpset5(ADX, DY, VDPpoint5(ASX, SY), LO);
            post_xxyy(256) break;
        case 6:
            pre_loop VDPpset6(ADX, DY, VDPpoint6(ASX, SY), LO);
            post_xxyy(512) break;
        case 7:
            pre_loop VDPpset7(ADX, DY, VDPpoint7(ASX, SY), LO);
            post_xxyy(512) break;
        case 8:
            pre_loop VDPpset8(ADX, DY, VDPpoint8(ASX, SY), LO);
            post_xxyy(256) break;
    }
...
/** CPU -> Vram                                             **/
/*************************************************************/
IRAM_ATTR void LmmcEngine(void) {
    if ((VDPStatus[2] & 0x80) != 0x80) {
...
IRAM_ATTR void HmmvEngine(void) {
    ...
    switch (ScrMode) {
        case 5:
            pre_loop *VDP_VRMP5(ADX, DY) = CL;
            post__x_y(256) break;
        case 6:
            pre_loop *VDP_VRMP6(ADX, DY) = CL;
            post__x_y(512) break;
        case 7:
            pre_loop *VDP_VRMP7(ADX, DY) = CL;
            post__x_y(512) break;
        case 8:
            pre_loop *VDP_VRMP8(ADX, DY) = CL;
            post__x_y(256) break;
    }
...
IRAM_ATTR void YmmmEngine(void) {
    ...
    switch (ScrMode) {
        case 5:
            pre_loop *VDP_VRMP5(ADX, DY) = *VDP_VRMP5(ADX, SY);
            post__xyy(256) break;
        case 6:
            pre_loop *VDP_VRMP6(ADX, DY) = *VDP_VRMP6(ADX, SY);
            post__xyy(512) break;
        case 7:
            pre_loop *VDP_VRMP7(ADX, DY) = *VDP_VRMP7(ADX, SY);
            post__xyy(512) break;
        case 8:
            pre_loop *VDP_VRMP8(ADX, DY) = *VDP_VRMP8(ADX, SY);
            post__xyy(256) break;
    }
...
IRAM_ATTR void VDPWrite(byte V) {
    ...
    if (__builtin_expect(ActiveVdpEngine && (VdpOpsCnt > 0), 0))
        RunActiveEngine();
}
...
IRAM_ATTR byte VDPDraw(byte Op) {
    ...
    switch (Op >> 4) {
        ...
        case CM_POINT:
            VDPStatus[2] &= 0xFE;
            ActiveVdpEngine = 0;
            VDPStatus[7] = VDP[44] = VDP_POINT(SM,
                                               VDP[32] + ((int)VDP[33] << 8),
                                               VDP[34] + ((int)VDP[35] << 8));
            return 1;
        case CM_PSET:
            ...
            return 1;
        case CM_SRCH:
            MMC.delta = GetVdpTimingValue(srch_timing);
            ActiveVdpEngine = CM_SRCH;
            break;
        case CM_LINE:
            MMC.delta = GetVdpTimingValue(line_timing);
            ActiveVdpEngine = CM_LINE;
            break;
        case CM_LMMV:
            MMC.delta = GetVdpTimingValue(lmmv_timing);
            ActiveVdpEngine = CM_LMMV;
            break;
        case CM_LMMM:
            MMC.delta = GetVdpTimingValue(lmmm_timing);
            ActiveVdpEngine = CM_LMMM;
            break;
        case CM_LMCM:
            MMC.delta = GetVdpTimingValue(lmmv_timing);
            ActiveVdpEngine = CM_LMCM;
            break;
        case CM_LMMC:
            MMC.delta = GetVdpTimingValue(lmmv_timing);
            ActiveVdpEngine = CM_LMMC;
            break;
        case CM_HMMV:
            MMC.delta = GetVdpTimingValue(hmmv_timing);
            ActiveVdpEngine = CM_HMMV;
            break;
        case CM_HMMM:
            MMC.delta = GetVdpTimingValue(hmmm_timing);
            ActiveVdpEngine = CM_HMMM;
            break;
        case CM_YMMM:
            MMC.delta = GetVdpTimingValue(ymmm_timing);
            ActiveVdpEngine = CM_YMMM;
            break;
        case CM_HMMC:
            MMC.delta = GetVdpTimingValue(hmmv_timing);
            ActiveVdpEngine = CM_HMMC;
            break;
        default:
            if (Verbose & 0x02)
    ...
    MMC.SX = VDP_R16(32) & 511;
    MMC.SY = VDP_R16(34) & 1023;
    MMC.DX = VDP_R16(36) & 511;
    MMC.DY = VDP_R16(38) & 1023;
    MMC.NY = VDP_R16(42) & 1023;
    ...
    if ((MMC.CM & 0x0C) == 0x0C) {
        ...
        MMC.NX = (VDP_R16(40) & 1023) / PPB[SM];
    } else {
        MMC.TX = VDP[45] & 0x04 ? -1 : 1;
        MMC.NX = VDP_R16(40) & 1023;
    }
    ...
    if (ActiveVdpEngine && (VdpOpsCnt > 0))
        RunActiveEngine();
...
IRAM_ATTR void LoopVDP(void) {
    if (__builtin_expect(VdpOpsCnt <= 0, 0)) {
        VdpOpsCnt += 12500;
        if (__builtin_expect(ActiveVdpEngine && (VdpOpsCnt > 0), 0))
            RunActiveEngine();
    } else {
        VdpOpsCnt = 12500;
        if (__builtin_expect(ActiveVdpEngine, 0))
            RunActiveEngine();
    }
}
...
IRAM_ATTR void LoopVDP(void)
{
  if(__builtin_expect(VdpOpsCnt<=0, 0))
  {
    // Даем блиттеру 1 000 000 тактов. Команды выполнятся за 1 вызов.
    VdpOpsCnt += 1000000; 
    if(__builtin_expect(ActiveVdpEngine&&(VdpOpsCnt>0), 0)) RunActiveEngine();
  }
  else
  {
    VdpOpsCnt = 1000000; // Мгновенное выполнение
    if(__builtin_expect(ActiveVdpEngine, 0)) RunActiveEngine();
  }
}
*/
