...
/**                                                         **/
/** v2.3: POSIX I/O (open/read/write/close)                 **/
/**    - Eliminates newlib FILE* buffer allocation in DRAM  **/
/**    - Saves ~4KB of critical internal RAM                **/
/**    - Uses existing IO_BUF_SIZE static buffers only      **/
/**                                                         **/
/*************************************************************/
...
/* Безопасное объявление внешних переменных из MSX.c */
...
// =========================================================================
// === БУФЕРИЗОВАННЫЙ ВВОД/ВЫВОД (POSIX — 0 байт DRAM) ====================
// =========================================================================

#define IO_BUF_SIZE 4096

static int StateFD = -1;  // POSIX file descriptor вместо FILE*
...
static inline void buf_putc(uint8_t c) {
    ...
    if (out_pos >= IO_BUF_SIZE) {
        if (StateFD >= 0) {
            write(StateFD, out_buf, out_pos);  // POSIX — 0 heap
        }
        out_pos = 0;
    }
}
...
static inline void buf_flush(void) {
    if (out_pos > 0 && StateFD >= 0) {
        write(StateFD, out_buf, out_pos);
        out_pos = 0;
    }
}
...
static inline int buf_getc(void) {
    if (in_pos >= in_avail) {
        if (StateFD >= 0) {
            int r = read(StateFD, in_buf, IO_BUF_SIZE);  // POSIX — 0 heap
            in_avail = (r > 0) ? r : 0;
        } else {
            in_avail = 0;
        }
...
static int skip_lz77(unsigned int len) {
    unsigned int i = 0;
    while (i < len) {
        int cmd_val = buf_getc();
        if (cmd_val == EOF) return 0;
        ...
        if (cmd <= 127) {
            unsigned int lit_len = cmd + 1;
            for (unsigned int j = 0; j < lit_len; j++) if (buf_getc() == EOF) return 0;
            i += lit_len;
        }
        else if (cmd <= 191) {
            if (buf_getc() == EOF) return 0;
            i += (cmd & 0x3F) + 3;
        }
        else if (cmd <= 223) {
            if (buf_getc() == EOF) return 0;
...
/*#define SkipSTRUCT(Name) \
  if (StateFD >= 0) { \
    if (lseek(StateFD, sizeof(Name), SEEK_CUR) < 0) { \
      for (unsigned int _j = 0; _j < sizeof(Name); _j++) buf_getc(); \
    } \
  } else { if (Size+sizeof(Name)>MaxSize) return(0); else Size+=sizeof(Name); }
*/
// Убираем lseek() и всегда используем внутренний цикл чтения buf_getc(), это гарантирует 100% синхронизацию буфера in_buf и указателей.
#define SkipSTRUCT(Name) \
  if (StateFD >= 0) { \
    for (unsigned int _j = 0; _j < sizeof(Name); _j++) buf_getc(); \
  } else { if (Size+sizeof(Name)>MaxSize) return(0); else Size+=sizeof(Name); }
...
#define SkipDATA_STRIDE(DataSize) \
  if (StateFD >= 0) { \
    int block_type = buf_getc(); \
    if (block_type == EOF) return(0); \
    if (!skip_lz77((DataSize))) return(0); \
...
// =========================================================================
// === SaveSTA / LoadSTA (POSIX open/close — 0 байт DRAM) =================
// =========================================================================
