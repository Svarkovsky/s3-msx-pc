/**
    ...
    SND_MODE_ACCURATE — Mitsutaka Okazaki (studio emulation)
                        Cycle-accurate chip emulation:
                        PSG  (emu2149) — AY-3-8910 with DC Blocker
                        OPLL (emu2413) — YM2413 FM-PAC, IRAM-optimized
                        SCC  (emu2212) — Konami SCC wave tables
                        Mix: 1:1:1 ratio + SoftClip limiter (18000)

    Stereo/Mono:
    ─────────────────────────────────────────────────────────
    msx_audio_stereo = false — Mono (default for VGA boards)
    msx_audio_stereo = true  — Stereo (PSG center, OPLL right, SCC left)
    ...
    - SoftClip limiter (threshold 18000, 4:1 compression ratio)
    - IRAM-optimized OPLL synthesis (no PSRAM tables, on-the-fly computation)
    - Frame-accurate buffer management for Retro-Go RTOS
    - Clipping protection with UNLIKELY branch hints

    Performance:
    ─────────────────────────────────────────────────────────
    Fast mode:     74-75 FPS (all games)
    Accurate mode: 74-75 FPS (PSG+SCC), 55-60 FPS (PSG+OPLL+SCC)
    DRAM free:     148 KB / PSRAM free: 6842 KB
...
//
SoundMode CurrentSndMode = SND_MODE_ACCURATE; // Mitsutaka Okazaki (studio emulation)
//SoundMode CurrentSndMode = SND_MODE_FAST;     // Marat Fayzullin (branchless meander)
//bool msx_audio_stereo = false;                // Mono by default
bool msx_audio_stereo = true;               // Stereo (when dual audio output)
//


// Указатели на студийные чипы (Okazaki)
void *psg_acc     = NULL;
...
unsigned int InitSound(unsigned int Rate,unsigned int Latency) {
    ...
    // === ИНИЦИАЛИЗАЦИЯ STUDIO ЧИПОВ (Okazaki) ===
    if (!psg_acc) {
    ...
    // ПРОВЕРКА ИНИЦИАЛИЗАЦИИ OPLL (FM-PAC)
    if (!studio_opll) {
        ...
        if (studio_opll) {
            ...
            // ЕСЛИ ПАМЯТЬ НЕ ВЫДЕЛИЛАСЬ, МЫ УВИДИМ ЭТО В ЛОГЕ!
    ...
    if (CurrentSndMode == SND_MODE_ACCURATE) {
        ...
        for(I = 0; I < Frames; I++) {
            ...
            if (psg_acc) {
                ...
                psg_val = (raw_psg - psg_dc) * 2;
            }

            if (studio_opll) {
                opll_val = OPLL_calc(studio_opll);  // opll_val остаётся как есть: ±1024
                // YM2413 выдаёт 11-битный звук (диапазон примерно ±1024), в то время как PSG и SCC могут раскачиваться до ±32767. 
                // Сам чип физически тише. Это не баг emu2413.c — это особенность реального железа. На реальном MSX OPLL тоже тише PSG. 
                // OPLL даже после усиления не превысит ±1536 — до клиппинга (32767) огромный запас.
                // opll_val = (opll_val * 3) >> 1;  // Усиление в 1.5x (50% громче) 
                opll_val = opll_val * 6;  // ±6144 — всё ещё безопасно
                // Чистое линейное усиление, работает одинаково для соло-записей и для игрового микса.                
            }

            if (studio_scc) {
                // Статическая переменная для хранения предыдущего состояния фильтра
                static int scc_lpf = 0; 

                int raw_scc = SCC_calc((SCC *)studio_scc) * 2;

                /* Однополюсный фильтр низких частот (срез ~9.5 кГц при частоте 32 кГц)
                   Математика: y[n] = y[n-1] + 0.5 * (x[n] - y[n-1])
                   Выполняется на Xtensa LX7 за 1-2 цикла процессора с помощью быстрого сдвига! */
                scc_lpf += (raw_scc - scc_lpf) >> 1; // Мягкий срез (~4.8 кГц): scc_lpf += (raw_scc - scc_lpf) >> 2;
            ...
            if (msx_audio_stereo) {
                out_l += (opll_val * 70) >> 8;
                out_r += (opll_val * 200) >> 8;
                out_l += (scc_val * 200) >> 8;
                out_r += (scc_val * 70) >> 8;
            }

            else {
                out_l += opll_val + scc_val;
                out_r += opll_val + scc_val;
            }

            out_l *= 1; // 1 усиление (х1)
            out_r *= 1; // 1 единичное усиление (0dB)

            Wave[I*2]   += SoftClip(out_l);
            Wave[I*2+1] += SoftClip(out_r);
        }
