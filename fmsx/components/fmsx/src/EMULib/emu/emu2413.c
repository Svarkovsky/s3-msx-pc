/***********************************************************************************
    ...
    - Removed 400+ KB of PSRAM lookup tables.
    - Replaced memory lookups with 1-cycle on-the-fly arithmetic and bit shifts.
    - Moved audio hot-paths to IRAM.
    - All lookup tables in DRAM (DRAM_ATTR).
    - FIXED: makeAdjustTable negative float UB.
    - FIXED: OPLL_calc -25% headroom.
    - FIXED: OPLL_setClock direct LFO calculation.
    - FIXED: calc_pg_dphase shift after multiply.
    - FIXED: makeSinTable half-sine perfect silence.
    - FIXED: Restored original phase-modulated rhythm synthesis.
    - FIXED: Replaced fake noise percussion with authentic FM percussion.
    - ADDED: Integrated 1-pole LPF (averaging with previous sample).
**************************************************************************************/
...
#define DB2LIN_AMP_BITS 11 // 11
//#define SLOT_AMP_BITS (DB2LIN_AMP_BITS)
#define SLOT_AMP_BITS 12
...
static void makeAdjustTable(void) {
    ...
    for ( i=1 ; i < 128 ; i++) {
        float v = (float)(1<<EG_BITS) - 1.0f - (float)(1<<EG_BITS) * logf((float)i) / logf(128.0f) ;
        if (v < 0.0f) v = 0.0f;
        AR_ADJUST_TABLE[i] = (e_uint32)v ;
    }
}
...
static void makeSinTable(void) {
    ...
    for( i = PG_WIDTH/2 ; i< PG_WIDTH ; i++ ) { halfsintable[i] = DB_MUTE ; }
}
...
static void makeTllTable(void) {
    ...
    static const e_uint32 kltable[16] = {0, 18, 24, 27, 30, 32, 33, 35, 36, 37, 38, 39, 39, 40, 41, 42};
    for(fnum=0; fnum<16; fnum++)
        for(block=0; block<8; block++)
            for(KL=0; KL<4; KL++) {
                if(KL==0) { tll_base[fnum][block][KL] = 0; }
                else {
                    tmp = kltable[fnum] - 6 * (7 - block);
...
void OPLL_dump2patch(const e_uint8 *dump, OPLL_PATCH *patch) {
    patch[0].AM = (dump[0]>>7)&1 ; patch[1].AM = (dump[1]>>7)&1 ;
    patch[0].PM = (dump[0]>>6)&1 ; patch[1].PM = (dump[1]>>6)&1 ;
    patch[0].EG = (dump[0]>>5)&1 ; patch[1].EG = (dump[1]>>5)&1 ;
    patch[0].KR = (dump[0]>>4)&1 ; patch[1].KR = (dump[1]>>4)&1 ;
    patch[0].ML = (dump[0])&15 ; patch[1].ML = (dump[1])&15 ;
    patch[0].KL = (dump[2]>>6)&3 ; patch[1].KL = (dump[3]>>6)&3 ;
    patch[0].TL = (dump[2])&63 ; patch[0].FB = (dump[3])&7 ;
    patch[0].WF = (dump[3]>>3)&1 ; patch[1].WF = (dump[3]>>4)&1 ;
    patch[0].AR = (dump[4]>>4)&15; patch[1].AR = (dump[5]>>4)&15;
    patch[0].DR = (dump[4])&15 ; patch[1].DR = (dump[5])&15 ;
    patch[0].SL = (dump[6]>>4)&15; patch[1].SL = (dump[7]>>4)&15;
    patch[0].RR = (dump[6])&15 ; patch[1].RR = (dump[7])&15 ;
}
...
INLINE static e_uint32 calc_pg_dphase(e_uint32 fnum, e_uint32 block, e_uint32 ML) {
    ...
    e_uint32 base = (fnum * mltable[ML]) << block;
    if (rate == 49716) { return base >> 2; }
    return (e_uint32)(((uint64_t)base * rate_adjust_mult) >> 18);
}
...
INLINE static void slotOn(OPLL_SLOT *slot) { slot->eg_mode = ATTACK ; slot->phase = 0 ; slot->eg_phase = 0 ; }
...
INLINE static void slotOff(OPLL_SLOT *slot) {
    ...
    UPDATE_EG(slot) ; // FIXED: immediate EG recalculation for RELEASE rate
}
...
INLINE static void setFnumber(OPLL *opll, e_int32 c, e_int32 fnum) {
    opll->CAR(c)->fnum = fnum ; opll->MOD(c)->fnum = fnum ;
}
INLINE static void setBlock(OPLL *opll, e_int32 c, e_int32 block) {
    opll->CAR(c)->block = block ; opll->MOD(c)->block = block ;
}
...
INLINE static void setRythmMode(OPLL *opll, e_int32 data) {
    ...
    if(data&32) {
        opll->ch[6]->patch_number = 16 ; opll->ch[7]->patch_number = 17 ; opll->ch[8]->patch_number = 18 ;
        setSlotPatch(opll->slot[SLOT_BD1], opll->patch[16*2+0]) ; setSlotPatch(opll->slot[SLOT_BD2], opll->patch[16*2+1]) ;
        setSlotPatch(opll->slot[SLOT_HH], opll->patch[17*2+0]) ; setSlotPatch(opll->slot[SLOT_SD], opll->patch[17*2+1]) ;
        opll->slot[SLOT_HH]->type = 1 ;
        setSlotPatch(opll->slot[SLOT_TOM], opll->patch[18*2+0]) ; setSlotPatch(opll->slot[SLOT_CYM], opll->patch[18*2+1]) ;
        opll->slot[SLOT_TOM]->type = 1 ;
    } else {
        setPatch(opll, 6, opll->reg[0x36]>>4) ; setPatch(opll, 7, opll->reg[0x37]>>4) ;
        opll->slot[SLOT_HH]->type = 0 ;
        setPatch(opll, 8, opll->reg[0x38]>>4) ; opll->slot[SLOT_TOM]->type = 0 ;
        if(!(opll->reg[0x26]&0x10)&&!(data&0x10)) { opll->slot[SLOT_BD1]->eg_mode = FINISH ; opll->slot[SLOT_BD2]->eg_mode = FINISH ; }
...
void OPLL_copyPatch(OPLL *opll, e_int32 num, OPLL_PATCH *patch) { memcpy(opll->patch[num],patch,sizeof(OPLL_PATCH)) ; }

static void OPLL_SLOT_reset(OPLL_SLOT *slot) {
    slot->sintbl = waveform[0] ; slot->phase = 0 ; slot->dphase = 0 ;
    slot->output[0] = 0 ; slot->output[1] = 0 ; slot->feedback = 0 ;
    slot->eg_mode = SETTLE ; slot->eg_phase = EG_DP_WIDTH ; slot->eg_dphase = 0 ;
    slot->rks = 0 ; slot->tll = 0 ; slot->sustine = 0 ;
    slot->fnum = 0 ; slot->block = 0 ; slot->volume = 0 ;
    slot->pgout = 0 ; slot->egout = 0 ; slot->patch = &null_patch ;
}

static OPLL_SLOT *OPLL_SLOT_new(void) { return heap_caps_malloc(sizeof(OPLL_SLOT), MALLOC_CAP_8BIT | MALLOC_CAP_INTERNAL); }
...
static OPLL_CH *OPLL_CH_new(void) {
    ...
    if((ch = heap_caps_malloc(sizeof(OPLL_CH), MALLOC_CAP_8BIT | MALLOC_CAP_INTERNAL)) == NULL) {
        OPLL_SLOT_delete(mod) ; OPLL_SLOT_delete(car) ; return NULL ;
    }
    mod->type = 0 ; car->type = 1 ; ch->mod = mod ; ch->car = car ;
    return ch ;
}
static void OPLL_CH_delete(OPLL_CH *ch) { OPLL_SLOT_delete(ch->mod) ; OPLL_SLOT_delete(ch->car) ; heap_caps_free(ch) ; }

OPLL *OPLL_new(void) {
    OPLL *opll ; OPLL_CH *ch[9] ; OPLL_PATCH *patch[19*2] ; e_int32 i, j ;
    ...
    for(i=0;i<19*2;i++) {
        if((patch[i]=heap_caps_calloc(1,sizeof(OPLL_PATCH),MALLOC_CAP_8BIT|MALLOC_CAP_INTERNAL))==NULL) {
            for(j=i;j>0;j--) heap_caps_free(patch[j-1]) ; heap_caps_free(opll) ; return NULL ;
        }
    }
    for(i=0;i<9;i++) {
        if((ch[i]=OPLL_CH_new())==NULL) {
            ...
            heap_caps_free(opll) ; return NULL ;
        }
    }
    for(i=0;i<19*2;i++) opll->patch[i]=patch[i] ;
    for(i=0;i<9;i++) { opll->ch[i]=ch[i] ; opll->slot[i*2+0]=opll->ch[i]->mod ; opll->slot[i*2+1]=opll->ch[i]->car ; }
    for(i=0;i<18;i++) { opll->slot[i]->plfo_am=&opll->lfo_am ; opll->slot[i]->plfo_pm=&opll->lfo_pm ; }
    opll->mask=0 ; OPLL_reset(opll) ; OPLL_reset_patch(opll,0) ;
    return opll ;
}
...
void OPLL_reset(OPLL *opll) {
    e_int32 i ;
    if(!opll) return ;
    opll->adr=0 ; opll->output[0]=0 ; opll->output[1]=0 ;
    opll->pm_phase=0 ; opll->am_phase=0 ; opll->noise_seed=0x7FFFFF ;
    opll->short_noise=0 ; opll->mask=0 ;
...
void OPLL_setClock(e_uint32 c, e_uint32 r) {
    ...
    makeDphaseARTable() ; makeDphaseDRTable() ;
    pm_dphase=(e_uint32)((float)PM_SPEED*PM_DP_WIDTH/(float)rate);
    am_dphase=(e_uint32)((float)AM_SPEED*AM_DP_WIDTH/(float)rate);
}

void OPLL_init(e_uint32 c, e_uint32 r) {
    makePmTable() ; makeAmTable() ; makeDB2LinTable() ; makeAdjustTable() ;
    makeTllTable() ; makeRksTable() ; makeSinTable() ; makeDefaultPatch() ;
    OPLL_setClock(c,r) ;
}
...
// ─── Original Okazaki noise generator (2 lines, zero latency) ───
INLINE static void update_noise(OPLL *opll) {
    if (opll->noise_seed & 1) opll->noise_seed ^= 0x800302;
    opll->noise_seed >>= 1;
}

// ─── Short noise bit for cymbal/hi-hat ───
static void update_short_noise(OPLL *opll) {
    const uint32_t pg_hh = opll->slot[SLOT_HH]->pgout;
    const uint32_t pg_cym = opll->slot[SLOT_CYM]->pgout;
...
INLINE static e_uint32 calc_envelope(OPLL_SLOT *slot) {
    static const e_uint32 SL[16]={0<<15,8<<15,16<<15,24<<15,32<<15,40<<15,48<<15,56<<15,64<<15,72<<15,80<<15,88<<15,96<<15,104<<15,112<<15,128<<15};
    e_uint32 egout ;
    switch(slot->eg_mode) {
        case ATTACK:
            slot->eg_phase+=slot->eg_dphase ;
            if(EG_DP_WIDTH&slot->eg_phase) { egout=0 ; slot->eg_phase=0 ; slot->eg_mode=DECAY ; UPDATE_EG(slot) ; }
            else egout=AR_ADJUST_TABLE[HIGHBITS(slot->eg_phase,EG_DP_BITS-EG_BITS)] ;
            break;
        case DECAY:
            slot->eg_phase+=slot->eg_dphase ; egout=HIGHBITS(slot->eg_phase,EG_DP_BITS-EG_BITS) ;
            if(slot->eg_phase>=SL[slot->patch->SL]) {
                ...
                UPDATE_EG(slot) ; egout=HIGHBITS(slot->eg_phase,EG_DP_BITS-EG_BITS) ;
            }
            break;
        case SUSHOLD:
            ...
            break;
        case SUSTINE: case RELEASE:
            slot->eg_phase+=slot->eg_dphase ; egout=HIGHBITS(slot->eg_phase,EG_DP_BITS-EG_BITS) ;
            if(egout>=(1<<EG_BITS)) { slot->eg_mode=FINISH ; egout=(1<<EG_BITS)-1 ; }
            break;
        case FINISH: default: egout=(1<<EG_BITS)-1 ; break;
    }
...
INLINE static e_int32 calc_slot_car(OPLL_SLOT *slot, e_int32 fm) {
    slot->egout=calc_envelope(slot) ; slot->pgout=calc_phase(slot) ; slot->output[1]=slot->output[0] ;
...
INLINE static e_int32 calc_slot_mod(OPLL_SLOT *slot) {
    e_int32 fm ; slot->output[1]=slot->output[0] ; slot->egout=calc_envelope(slot) ; slot->pgout=calc_phase(slot) ;
    if(UNLIKELY(slot->egout>=(DB_MUTE-1))) { slot->output[0]=0 ; }
...
INLINE static e_int32 calc_slot_tom(OPLL_SLOT *slot) {
    slot->egout=calc_envelope(slot) ; slot->pgout=calc_phase(slot) ;
    if(UNLIKELY(slot->egout>=(DB_MUTE-1))) return 0 ;
    return DB2LIN_TABLE[slot->sintbl[slot->pgout]+slot->egout] ;
}

// ─── Original phase-modulated snare drum ───
INLINE static e_int32 calc_slot_snare(OPLL_SLOT *slot, e_uint32 noise) {
    slot->egout=calc_envelope(slot) ; slot->pgout=calc_phase(slot) ;
    if(UNLIKELY(slot->egout>=(DB_MUTE-1))) return 0 ;
    e_uint32 phase;
    if(BIT(slot->pgout,PG_BITS-2)) phase=(noise&1)?0x180:0x100;
    else phase=(noise&1)?0x000:0x080;
    return DB2LIN_TABLE[slot->sintbl[phase]+slot->egout];
}

// ─── Original phase-modulated cymbal ───
INLINE static e_int32 calc_slot_cym(OPLL_SLOT *slot, e_uint32 short_noise) {
    slot->egout=calc_envelope(slot) ;
    if(UNLIKELY(slot->egout>=(DB_MUTE-1))) return 0 ;
    e_uint32 phase=short_noise?0x180:0x080;
    return DB2LIN_TABLE[slot->sintbl[phase]+slot->egout];
}

// ─── Original phase-modulated hi-hat ───
INLINE static e_int32 calc_slot_hat(OPLL_SLOT *slot, e_uint32 short_noise, e_uint32 noise) {
    slot->egout=calc_envelope(slot) ;
    if(UNLIKELY(slot->egout>=(DB_MUTE-1))) return 0 ;
    e_uint32 phase;
    if(short_noise) phase=(noise&1)?0x168:0x11A;
    else phase=(noise&1)?0x01A:0x068;
    return DB2LIN_TABLE[slot->sintbl[phase]+slot->egout];
}

EMU2413_API IRAM_ATTR e_int16 OPLL_calc(OPLL *opll) {
    e_int32 inst=0, perc=0, out=0 ; e_int32 i ;
    update_ampm(opll) ; update_noise(opll) ;
    ...
    if(!opll->rythm_mode) {
        ...
        opll->slot[SLOT_HH]->pgout=calc_phase(opll->slot[SLOT_HH]) ;
        opll->slot[SLOT_CYM]->pgout=calc_phase(opll->slot[SLOT_CYM]) ;
        ...
        if(!(opll->mask&OPLL_MASK_HH)&&(opll->slot[SLOT_HH]->eg_mode!=FINISH))
            perc+=calc_slot_hat(opll->slot[SLOT_HH],opll->short_noise,opll->noise_seed) ;
        if(!(opll->mask&OPLL_MASK_SD)&&(opll->CAR(7)->eg_mode!=FINISH))
            perc+=calc_slot_snare(opll->CAR(7),opll->noise_seed) ;
        ...
        if(!(opll->mask&OPLL_MASK_CYM)&&(opll->CAR(8)->eg_mode!=FINISH))
            perc+=calc_slot_cym(opll->CAR(8),opll->short_noise) ;
    }
    ...
    // Integrated 1-pole LPF (averaging with previous sample)
    e_int32 filtered=(out+opll->output[0])>>1 ;
    opll->output[0]=out ;

    return (e_int16)filtered ;
}

e_uint32 OPLL_setMask(OPLL *opll, e_uint32 mask) { e_uint32 ret=0; if(opll){ret=opll->mask;opll->mask=mask;} return ret; }
e_uint32 OPLL_toggleMask(OPLL *opll, e_uint32 mask) { e_uint32 ret=0; if(opll){ret=opll->mask;opll->mask^=mask;} return ret; }

void OPLL_writeReg(OPLL *opll, e_uint32 reg, e_uint32 data) {
    e_int32 i,v,ch ; data&=0xff ; reg&=0x3f ;
    switch(reg) {
        case 0x00: opll->patch[0]->AM=(data>>7)&1; opll->patch[0]->PM=(data>>6)&1; opll->patch[0]->EG=(data>>5)&1; opll->patch[0]->KR=(data>>4)&1; opll->patch[0]->ML=data&15;
            for(i=0;i<9;i++) if(opll->ch[i]->patch_number==0) { UPDATE_PG(opll->MOD(i)); UPDATE_RKS(opll->MOD(i)); UPDATE_EG(opll->MOD(i)); } break;
        case 0x01: opll->patch[1]->AM=(data>>7)&1; opll->patch[1]->PM=(data>>6)&1; opll->patch[1]->EG=(data>>5)&1; opll->patch[1]->KR=(data>>4)&1; opll->patch[1]->ML=data&15;
            for(i=0;i<9;i++) if(opll->ch[i]->patch_number==0) { UPDATE_PG(opll->CAR(i)); UPDATE_RKS(opll->CAR(i)); UPDATE_EG(opll->CAR(i)); } break;
        case 0x02: opll->patch[0]->KL=(data>>6)&3; opll->patch[0]->TL=data&63;
            for(i=0;i<9;i++) if(opll->ch[i]->patch_number==0) UPDATE_TLL(opll->MOD(i)); break;
        case 0x03: opll->patch[1]->KL=(data>>6)&3; opll->patch[1]->WF=(data>>4)&1; opll->patch[0]->WF=(data>>3)&1; opll->patch[0]->FB=data&7;
            for(i=0;i<9;i++) if(opll->ch[i]->patch_number==0) { UPDATE_WF(opll->MOD(i)); UPDATE_WF(opll->CAR(i)); } break;
        case 0x04: opll->patch[0]->AR=(data>>4)&15; opll->patch[0]->DR=data&15;
            for(i=0;i<9;i++) if(opll->ch[i]->patch_number==0) UPDATE_EG(opll->MOD(i)); break;
        case 0x05: opll->patch[1]->AR=(data>>4)&15; opll->patch[1]->DR=data&15;
            for(i=0;i<9;i++) if(opll->ch[i]->patch_number==0) UPDATE_EG(opll->CAR(i)); break;
        case 0x06: opll->patch[0]->SL=(data>>4)&15; opll->patch[0]->RR=data&15;
            for(i=0;i<9;i++) if(opll->ch[i]->patch_number==0) UPDATE_EG(opll->MOD(i)); break;
        case 0x07: opll->patch[1]->SL=(data>>4)&15; opll->patch[1]->RR=data&15;
            for(i=0;i<9;i++) if(opll->ch[i]->patch_number==0) UPDATE_EG(opll->CAR(i)); break;
        case 0x0e:
            if(opll->rythm_mode) {
                ...
                opll->slot_on_flag[SLOT_BD1]=opll->reg[0x26]&0x10; opll->slot_on_flag[SLOT_BD2]=opll->reg[0x26]&0x10;
                opll->slot_on_flag[SLOT_SD]=opll->reg[0x27]&0x10; opll->slot_on_flag[SLOT_HH]=opll->reg[0x27]&0x10;
                opll->slot_on_flag[SLOT_TOM]=opll->reg[0x28]&0x10; opll->slot_on_flag[SLOT_CYM]=opll->reg[0x28]&0x10;
            }
            ...
            UPDATE_ALL(opll->MOD(6)); UPDATE_ALL(opll->CAR(6)); UPDATE_ALL(opll->MOD(7));
            UPDATE_ALL(opll->CAR(7)); UPDATE_ALL(opll->MOD(8)); UPDATE_ALL(opll->CAR(8));
            break;
        case 0x0f: break;
        case 0x10:case 0x11:case 0x12:case 0x13:case 0x14:case 0x15:case 0x16:case 0x17:case 0x18:
            ch=reg-0x10; setFnumber(opll,ch,data+((opll->reg[0x20+ch]&1)<<8));
            UPDATE_ALL(opll->MOD(ch)); UPDATE_ALL(opll->CAR(ch));
            break;
        case 0x20:case 0x21:case 0x22:case 0x23:case 0x24:case 0x25:case 0x26:case 0x27:case 0x28:
            ch=reg-0x20; setFnumber(opll,ch,((data&1)<<8)+opll->reg[0x10+ch]); setBlock(opll,ch,(data>>1)&7);
        ...
        case 0x30:case 0x31:case 0x32:case 0x33:case 0x34:case 0x35:case 0x36:case 0x37:case 0x38:
            i=(data>>4)&15; v=data&15;
            if((opll->rythm_mode)&&(reg>=0x36)) {
                if(reg==0x37) setSlotVolume(opll->MOD(7),i<<2);
                else if(reg==0x38) setSlotVolume(opll->MOD(8),i<<2);
            } else setPatch(opll,reg-0x30,i) ;
            setVolume(opll,reg-0x30,v<<2); UPDATE_ALL(opll->MOD(reg-0x30)); UPDATE_ALL(opll->CAR(reg-0x30));
            break;
    }
