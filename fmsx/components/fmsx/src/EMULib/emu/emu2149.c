...
EMU2149_API void PSG_set_quality(e_uint32 q)
{
  if(q)
    ...
    base_incr = (e_uint32)(((uint64_t)clk << GETA_BITS) / (16 * (uint64_t)rate)) ;
...
EMU2149_API PSG *PSG_new(void)
{
	...
	psg = (PSG *)heap_caps_malloc(sizeof(PSG), MALLOC_CAP_8BIT | MALLOC_CAP_INTERNAL);
...
EMU2149_API void PSG_reset(PSG *psg)
{
  ...
  psg->adr = 0 ;
	...
	psg->realstep = (e_uint32)(((uint64_t)1<<31) / rate) ;
	psg->psgstep =  (e_uint32)(((uint64_t)1<<31) / (clk / 16)) ;
	psg->psgtime = 0 ;
...
{
	if(psg != NULL) heap_caps_free(psg) ;
}
...
EMU2149_API void PSG_writeReg(PSG *psg, e_uint32 reg, e_uint32 val)
{
	...
	switch(reg)
  {
    ...
    /* Возвращено оригинальное "sticky" поведение: генератор фазы огибающей 
       (env_ptr) продолжает непрерывно тикать в фоне, даже если каналы её временно не используют */
...
INLINE static e_int16 calc(PSG *psg)
{
  ...
  {
    psg->count[i] += incr ;
    /* Fix: O(1) overflows — корректный перенос фазы */
    if(psg->count[i] >= 0x1000)
    {
       if(psg->freq[i] > 1)
       {
           e_uint32 overflows = (psg->count[i] - 0x1000) / psg->freq[i] + 1;
           if(overflows & 1) psg->edge[i] = !psg->edge[i];
           psg->count[i] -= overflows * psg->freq[i];
       }
       else
       {
         psg->edge[i] = 1 ;
