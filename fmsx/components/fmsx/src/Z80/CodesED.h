...
/**                                                         **/
/** Optimization (2026): LDIR/LDDR use native C loops       **/
/** to batch-copy bytes instead of single-stepping.         **/
/** Fixed: INDR/OTDR logic (original Marat's bug).          **/
/*************************************************************/
...
case INDR:
  WrZ80(R->HL.W--,InZ80(R->BC.W));
  if(--R->BC.B.h) { R->AF.B.l=N_FLAG;R->ICount-=21;R->PC.W-=2; }  // FIXED: was if(!--R->BC.B.h)
  else            { R->AF.B.l=Z_FLAG|N_FLAG;R->ICount-=16; }
  break;
...
case LDIR:
{
    ...
    if (R->BC.W)  // Если BC != 0 (включая случай BC=0 = 65536)
    {
        do {
            WrZ80(R->DE.W++, RdZ80(R->HL.W++));
            R->BC.W--;
        ...
        if (R->BC.W) {
            // Прервались по ICount
            R->AF.B.l |= P_FLAG;
            R->PC.W -= 2;
        } else {
            // Завершено: последняя итерация 16 тактов, а не 21
            R->ICount += 5;
        }
    }
    else
    {
        // BC=0 не должен случаться для LDIR, но обработаем
        R->ICount -= 16;
    }
    break;
}
