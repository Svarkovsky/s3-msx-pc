/**
    Z80: portable Z80 emulator — Z80.c

    Z80 CPU emulation core. Requires RdZ80(), WrZ80(), InZ80(),
    OutZ80(), LoopZ80(), and PatchZ80() functions provided by
    the emulated machine's architecture.
    ...
    - The massive switch/case dispatcher has been replaced with
     a static jump table op_table[256] (1 KB). The NEXT_OP()
     macro fetches the next opcode and jumps directly to its
     handler via computed goto (goto *op_table[I]), bypassing
     the central dispatcher entirely. Xtensa LX7 executes this
     with zero branch misprediction penalty on the hot path.

    -- Register Caching (Level 1) --

    - Program Counter (PC) and Instruction Count (ICount) are
     pinned to hardware 32-bit registers via register uint32_t.
     All memory accesses through R->PC.W and R->ICount in the
     main dispatch loop have been eliminated. The Z80_PC macro
     is redefined to map to the local PC variable.

    - Before calling prefix handlers (CB/DD/ED/FD) or entering
     check_ints, the cached registers are synced to the Z80
     structure (R->PC.W = PC; R->ICount = ICount). After the
     call, they are restored. This maintains full compatibility
     with MSX-DOS, save states, and interrupt handling.

    -- IRAM Placement --

    - RunZ80 and ExecZ80 placed in .iram1.text for zero-latency
     instruction fetch on every opcode dispatch.

    - Prefix handlers (CodesCB, CodesDD, CodesED, CodesFD)
     moved to IRAM via IRAM_ATTR to eliminate Flash cache
     misses during prefix instruction execution.

    -- Branch Prediction --

    - LIKELY()/UNLIKELY() macros (__builtin_expect) added to
     hot paths: NEXT_OP dispatch, interrupt checks, and
     conditional EI/HALT handling.

    -- Macro Surgery --

    - Base macros (M_CALL, M_RET, M_JP, M_JR, M_RST, M_LDWORD)
     rewritten to operate on the fast local 32-bit PC register
     instead of the 16-bit R->PC.W structure member.

    -- CodesCB Inlining (Tier 1, 2026) --

    - CodesCB force-inlined via __attribute__((always_inline))
     to eliminate call8 overhead on Xtensa window rotation.
     All register cache sync/restore for CB prefix removed.
    
    -- Smart Turbo (Tier 2, 2026) --

    - Added global uint8_t TurboMode variable. When enabled,
     instruction cycle costs are halved (>> 1) for all opcodes
     except I/O instructions (0xD3, 0xDB) and ED-prefix block
     transfers and port operations (0x40-0x7F, 0xA0-0xBF).
     This preserves VDP synchronization, disk/tape loader
     timing, and PSG audio while doubling throughput for pure
     CPU operations. Benchmarked at ~8.23 MHz effective Z80
     frequency on ESP32-S3 when TurboMode=1.

    WARNING: This version is under active testing. Please report
    any emulation bugs, crashes, or save state corruption to:
...
/* TurboMode: 0 = normal, 1 = 2x turbo (preserves I/O timing) */
uint8_t TurboMode = 0;

/** INLINE & ZERO-COST OPTIMIZATIONS *************************/
#ifdef __C99__
...
/* Branch prediction macros (Zero memory cost) */
...
#define FAST_RDOP
extern byte *RAM[];
__attribute__((always_inline)) static inline byte OpZ80(word A) {
...
/* ========================================================= */
/* MACRO TRICK: Mapping Z80_PC to R->PC.W for legacy headers */
/* ========================================================= */
#define Z80_PC R->PC.W

#define M_CALL         \
  J.B.l=OpZ80(Z80_PC++);J.B.h=OpZ80(Z80_PC++); \
  WrZ80(--R->SP.W, (Z80_PC) >> 8); WrZ80(--R->SP.W, (Z80_PC) & 0xFF); \
  Z80_PC=J.W; \
  JumpZ80(J.W)

#define M_JP  J.B.l=OpZ80(Z80_PC++);J.B.h=OpZ80(Z80_PC);Z80_PC=J.W;JumpZ80(J.W)
#define M_JR  Z80_PC+=(offset)OpZ80(Z80_PC)+1;JumpZ80(Z80_PC)
#define M_RET Z80_PC=(OpZ80(R->SP.W++)&0xFF);Z80_PC|=(OpZ80(R->SP.W++)<<8);JumpZ80(Z80_PC)

#define M_RST(Ad)      \
  WrZ80(--R->SP.W, (Z80_PC) >> 8); WrZ80(--R->SP.W, (Z80_PC) & 0xFF); Z80_PC=Ad; JumpZ80(Ad)

#define M_LDWORD(Rg)   \
  R->Rg.B.l=OpZ80(Z80_PC++);R->Rg.B.h=OpZ80(Z80_PC++)
...
/* ========================================================= */
/* ENUMS REQUIRED BY LEGACY HEADERS (CodesCB, CodesDD, etc.) */
/* ========================================================= */
enum Codes {
...
__attribute__((always_inline)) static inline void CodesCB(register Z80 *R, uint32_t *PC, int *ICount) {
    register byte I;
    I = OpZ80((*PC)++);
    if (TurboMode) {
        *ICount -= (CyclesCB[I] >> 1);
        ...
        default:
            if(R->TrapBadOps)
                ...
                (
                    ...
                    (unsigned long)(R->User), OpZ80(*PC - 1), (unsigned int)(*PC - 2)
                );
    }
}
...
IRAM_ATTR static void CodesED(register Z80 *R) {
    ...
    I=OpZ80(R->PC.W++);
    if (TurboMode) {
        if (UNLIKELY((I >= 0x40 && I <= 0x7F) || (I >= 0xA0 && I <= 0xBF))) {
            R->ICount -= CyclesED[I];
        } else {
            R->ICount -= (CyclesED[I] >> 1);
        }
    } else {
        R->ICount -= CyclesED[I];
    }
...
void IntZ80(Z80 *R,word Vector) {
    if(UNLIKELY(R->IFF&IFF_HALT)) { R->PC.W++; R->IFF&=~IFF_HALT; }
...
/* ========================================================= */
/* MACRO TRICK: Mapping Z80_PC to local PC for RunZ80        */
/* ========================================================= */
#undef Z80_PC
#define Z80_PC PC

/** RunZ80() *************************************************/
/** OPTIMIZED: COMPUTED GOTO AND REGISTER CACHING           **/
/*************************************************************/
#ifndef EXECZ80
word __attribute__((section(".iram1.text"))) RunZ80(register Z80 * restrict R) {
    ...
    /* Caching PC and ICount in physical 32-bit registers */
    uint32_t PC = R->PC.W;
    int ICount = R->ICount;

    /* The core Jump Table for Computed Goto */
    static const void *const op_table[256] Z80_TABLE_ATTR = {
        &&op_NOP, &&op_LD_BC_WORD, &&op_LD_xBC_A, &&op_INC_BC, &&op_INC_B, &&op_DEC_B, &&op_LD_B_BYTE, &&op_RLCA,
        &&op_EX_AF_AF, &&op_ADD_HL_BC, &&op_LD_A_xBC, &&op_DEC_BC, &&op_INC_C, &&op_DEC_C, &&op_LD_C_BYTE, &&op_RRCA,
        &&op_DJNZ, &&op_LD_DE_WORD, &&op_LD_xDE_A, &&op_INC_DE, &&op_INC_D, &&op_DEC_D, &&op_LD_D_BYTE, &&op_RLA,
        &&op_JR, &&op_ADD_HL_DE, &&op_LD_A_xDE, &&op_DEC_DE, &&op_INC_E, &&op_DEC_E, &&op_LD_E_BYTE, &&op_RRA,
        &&op_JR_NZ, &&op_LD_HL_WORD, &&op_LD_xWORD_HL, &&op_INC_HL, &&op_INC_H, &&op_DEC_H, &&op_LD_H_BYTE, &&op_DAA,
        &&op_JR_Z, &&op_ADD_HL_HL, &&op_LD_HL_xWORD, &&op_DEC_HL, &&op_INC_L, &&op_DEC_L, &&op_LD_L_BYTE, &&op_CPL,
        &&op_JR_NC, &&op_LD_SP_WORD, &&op_LD_xWORD_A, &&op_INC_SP, &&op_INC_xHL, &&op_DEC_xHL, &&op_LD_xHL_BYTE, &&op_SCF,
        &&op_JR_C, &&op_ADD_HL_SP, &&op_LD_A_xWORD, &&op_DEC_SP, &&op_INC_A, &&op_DEC_A, &&op_LD_A_BYTE, &&op_CCF,
        &&op_LD_B_B, &&op_LD_B_C, &&op_LD_B_D, &&op_LD_B_E, &&op_LD_B_H, &&op_LD_B_L, &&op_LD_B_xHL, &&op_LD_B_A,
        &&op_LD_C_B, &&op_LD_C_C, &&op_LD_C_D, &&op_LD_C_E, &&op_LD_C_H, &&op_LD_C_L, &&op_LD_C_xHL, &&op_LD_C_A,
        &&op_LD_D_B, &&op_LD_D_C, &&op_LD_D_D, &&op_LD_D_E, &&op_LD_D_H, &&op_LD_D_L, &&op_LD_D_xHL, &&op_LD_D_A,
        &&op_LD_E_B, &&op_LD_E_C, &&op_LD_E_D, &&op_LD_E_E, &&op_LD_E_H, &&op_LD_E_L, &&op_LD_E_xHL, &&op_LD_E_A,
        &&op_LD_H_B, &&op_LD_H_C, &&op_LD_H_D, &&op_LD_H_E, &&op_LD_H_H, &&op_LD_H_L, &&op_LD_H_xHL, &&op_LD_H_A,
        &&op_LD_L_B, &&op_LD_L_C, &&op_LD_L_D, &&op_LD_L_E, &&op_LD_L_H, &&op_LD_L_L, &&op_LD_L_xHL, &&op_LD_L_A,
        &&op_LD_xHL_B, &&op_LD_xHL_C, &&op_LD_xHL_D, &&op_LD_xHL_E, &&op_LD_xHL_H, &&op_LD_xHL_L, &&op_HALT, &&op_LD_xHL_A,
        &&op_LD_A_B, &&op_LD_A_C, &&op_LD_A_D, &&op_LD_A_E, &&op_LD_A_H, &&op_LD_A_L, &&op_LD_A_xHL, &&op_LD_A_A,
        &&op_ADD_B, &&op_ADD_C, &&op_ADD_D, &&op_ADD_E, &&op_ADD_H, &&op_ADD_L, &&op_ADD_xHL, &&op_ADD_A,
        &&op_ADC_B, &&op_ADC_C, &&op_ADC_D, &&op_ADC_E, &&op_ADC_H, &&op_ADC_L, &&op_ADC_xHL, &&op_ADC_A,
        &&op_SUB_B, &&op_SUB_C, &&op_SUB_D, &&op_SUB_E, &&op_SUB_H, &&op_SUB_L, &&op_SUB_xHL, &&op_SUB_A,
        &&op_SBC_B, &&op_SBC_C, &&op_SBC_D, &&op_SBC_E, &&op_SBC_H, &&op_SBC_L, &&op_SBC_xHL, &&op_SBC_A,
        &&op_AND_B, &&op_AND_C, &&op_AND_D, &&op_AND_E, &&op_AND_H, &&op_AND_L, &&op_AND_xHL, &&op_AND_A,
        &&op_XOR_B, &&op_XOR_C, &&op_XOR_D, &&op_XOR_E, &&op_XOR_H, &&op_XOR_L, &&op_XOR_xHL, &&op_XOR_A,
        &&op_OR_B, &&op_OR_C, &&op_OR_D, &&op_OR_E, &&op_OR_H, &&op_OR_L, &&op_OR_xHL, &&op_OR_A,
        &&op_CP_B, &&op_CP_C, &&op_CP_D, &&op_CP_E, &&op_CP_H, &&op_CP_L, &&op_CP_xHL, &&op_CP_A,
        &&op_RET_NZ, &&op_POP_BC, &&op_JP_NZ, &&op_JP, &&op_CALL_NZ, &&op_PUSH_BC, &&op_ADD_BYTE, &&op_RST00,
        &&op_RET_Z, &&op_RET, &&op_JP_Z, &&op_PFX_CB, &&op_CALL_Z, &&op_CALL, &&op_ADC_BYTE, &&op_RST08,
        &&op_RET_NC, &&op_POP_DE, &&op_JP_NC, &&op_OUTA, &&op_CALL_NC, &&op_PUSH_DE, &&op_SUB_BYTE, &&op_RST10,
        &&op_RET_C, &&op_EXX, &&op_JP_C, &&op_INA, &&op_CALL_C, &&op_PFX_DD, &&op_SBC_BYTE, &&op_RST18,
        &&op_RET_PO, &&op_POP_HL, &&op_JP_PO, &&op_EX_HL_xSP, &&op_CALL_PO, &&op_PUSH_HL, &&op_AND_BYTE, &&op_RST20,
        &&op_RET_PE, &&op_LD_PC_HL, &&op_JP_PE, &&op_EX_DE_HL, &&op_CALL_PE, &&op_PFX_ED, &&op_XOR_BYTE, &&op_RST28,
        &&op_RET_P, &&op_POP_AF, &&op_JP_P, &&op_DI, &&op_CALL_P, &&op_PUSH_AF, &&op_OR_BYTE, &&op_RST30,
        &&op_RET_M, &&op_LD_SP_HL, &&op_JP_M, &&op_EI, &&op_CALL_M, &&op_PFX_FD, &&op_CP_BYTE, &&op_RST38
    };
...
#define NEXT_OP() \
    ...
    /* Include our optimized Threaded Code */
#include "Codes.h"

    /* Handle prefixes as sub-functions, safely syncing cached registers */
op_PFX_CB:
    CodesCB(R, &PC, &ICount);   //  передаем указатели, без sync/restore
    NEXT_OP();
...
check_ints:
    /* Time to handle interrupts / screen refresh */
    R->PC.W = PC;
    R->ICount = ICount;
    ...
    if(R->IFF&IFF_EI) {
        ...
        if(LIKELY(R->ICount>0)) { J.W=R->IRequest; }
        else {
            J.W=LoopZ80(R);
            R->ICount+=R->IPeriod;

            if(J.W==INT_NONE) { J.W=R->IRequest; }
        }
    }
    else {
        J.W=LoopZ80(R);
    ...
    if(UNLIKELY(J.W==INT_QUIT)) { return(R->PC.W); }

    if(UNLIKELY(J.W!=INT_NONE)) { IntZ80(R,J.W); }
...
check_ints:
    ...
    if(!(R->IFF&IFF_EI)) { return(R->ICount); }
        ...
        if(UNLIKELY((R->IRequest!=INT_NONE)&&(R->IRequest!=INT_QUIT))) { IntZ80(R,R->IRequest); }
    }
