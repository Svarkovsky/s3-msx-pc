...
// Палитры MSX (к ним VDP обращается миллионы раз в секунду)
    ...
    if (__builtin_expect(has_ctrl, 0))
    {
        for (int i = 2; i < 8; i++)
        {
            if (local_report[i] == 0x3F) {
                ...
                if (path) { 
                    rg_gui_draw_message("Saving Disk..."); // Сообщение
            ...
            if (local_report[i] == 0x40) {
                ...
                if (path) { 
                    rg_gui_draw_message("Saving Disk...");
                    rg_system_tick(0);
                    MSX_SaveDisks(); 
                    ChangeDisk(1, path); 
...
void Keyboard(void)
{
    ...
    if (PendingLoadSTA) { 
        ...
        free(PendingLoadSTA); 
        PendingLoadSTA = NULL; 
        extern void ResetStudioChips(void); // БРАСЫВАЕМ СТУДИЙНЫЕ ЧИПЫ OKAZAKI 
        ResetStudioChips();
...
int ShowVideo(void)
{
    ...
    rg_system_tick(0); // Ограничитель кадров силами Retro-Go (55 Гц)
    return 1;
}
...
void app_main(void)
{
    ...
    rg_system_set_tick_rate(55); // 55
    
    // Читаем модель, сохранённую лаунчером в msx.json
    ...
    // Читаем настройку автосохранения
    ...
    const char *argv[] = {
        "fmsx",
        msx_mode,              // вместо жёсткого "-msx2"
        // "-msx2",              // или "-msx2+",          -msx1/-msx2/-msx2+  - Select MSX model [-msx2]
        "-ram", "256",        // 16 * 16KB = 256 KB          256 * 16 = 4 MB
...
/*    if (auto_fire) argv[argc++] = "-auto";
    if (rg_extension_match(app->romPath, "dsk")) argv[argc++] = "-diska";
    else if (rg_extension_match(app->romPath, "cas")) argv[argc++] = "-tape";     // .CAS
...
*/
        ...
        argv[argc++] = app->romPath;
//
    audioQueue = rg_task_create("audioTask", &audioTask, NULL, 4096, RG_TASK_PRIORITY_3, 1); // достаточно такого
